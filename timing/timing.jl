...
@testset "OMEinsum with $atype{$dtype} " for atype in [ROCArray], dtype in [ComplexF64]
    ...
    # @btime simple_eig(FL -> FLmap(FL, $AL, conj($AL), $ipeps), $FL)
    AMDGPU.@time AMDGPU.@sync begin
        FLmap(FL, AL, conj(AL[:,:,:,1:div(χ,2)]), M)
        FLmap(FL, AL, conj(AL[:,:,:,div(χ,2)+1:end]), M)
    end
    # AMDGPU.@time AMDGPU.@sync for _ in 1:1
    #     FLmap(FL, AL, conj(AL), M)
    # end
end
