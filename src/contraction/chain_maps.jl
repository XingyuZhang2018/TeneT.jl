...
# only the ORDER permuted (asserted). The slice2d forward/rrule always run the 2M
# chain, so NO _1M twin is needed.
const FRMAP_LEG5_SLICE2D_CHAIN = tensor_chain(((:d,:g,:h,:l), (:a,:b,:c,:d), (:e,:j,:g,:b,:p), (:f,:k,:h,:c,:p), (:i,:j,:k,:l)), (:a,:e,:f,:i))
@assert Set(FRMAP_LEG5_SLICE2D_CHAIN.ops) == Set(FRMAP_LEG5_CHAIN.ops) && FRMAP_LEG5_SLICE2D_CHAIN.out == FRMAP_LEG5_CHAIN.out "FRMAP_LEG5_SLICE2D_CHAIN must be an operand permutation of FRMAP_LEG5_CHAIN (identical legs/out, reordered only)"
...
# only the ORDER permuted (asserted). The slice2d forward/rrule always run the 2M
# chain, so NO _1M twin is needed.
const ACDMAP_LEG5_SLICE2D_CHAIN = tensor_chain(((:a,:e,:f,:i), (:i,:j,:k,:l), (:e,:j,:g,:b,:p), (:f,:k,:h,:c,:p), (:d,:g,:h,:l)), (:a,:b,:c,:d))
@assert Set(ACDMAP_LEG5_SLICE2D_CHAIN.ops) == Set(ACDMAP_LEG5_CHAIN.ops) && ACDMAP_LEG5_SLICE2D_CHAIN.out == ACDMAP_LEG5_CHAIN.out "ACDMAP_LEG5_SLICE2D_CHAIN must be an operand permutation of ACDMAP_LEG5_CHAIN (identical legs/out, reordered only)"
