...
function plot_lattice_obs(e_dict, m_dict, lattice_type, pattern::Matrix{Int};
    ...
    ax = Axis(fig[1, 1]; title="Lattice Observables  ($info)", aspect=DataAspect(),
...
# ============================================================================
# Honeycomb merge lattice visualization
# ============================================================================

function plot_lattice_obs(e_dict, m_dict, lattice_type::Honeycomb{:merge}, pattern::Matrix{Int};
                          save_path::String, save_format::String="png", χ::Int=0, n_repeat::Int=3,
                          e_scalar::Real=NaN, mag_scalar::Real=NaN, ξ_scalar::Real=NaN, S::Real=0.5)
    isdir(save_path) || mkpath(save_path)
    Ni, Nj = size(pattern)
    ...
    for di in 0:(n_repeat-1), dj in 0:(n_repeat-1)
        for ci in 1:Ni, cj in 1:Nj
            ...
            orig = (di == 0 && dj == 0)
            for k in 1:2
                all_coords[(gi, gj, k)] = _honeycomb_merge_site_xy(gi, gj, k)
    ...
    isnan(mag_scalar) || (info *= "  |M|=$(round(mag_scalar; sigdigits=6))")
    isnan(ξ_scalar) || (info *= "  ξ=$(round(ξ_scalar; sigdigits=6))")
    ax = Axis(fig[1, 1]; title="Honeycomb Merge Observables  ($info)", aspect=DataAspect(),
              backgroundcolor=:white)
    hidedecorations!(ax)
    hidespines!(ax)

    _draw_honeycomb_merge_bonds!(ax, all_coords, e_dict, pattern, unique_sites,
                                 Ni, Nj, n_repeat, e_min, e_max)
...
function plot_lattice_obs(e_dict, m_dict, lattice_type::Kagome{:merge}, pattern::Matrix{Int};
    ...
    isnan(mag_scalar) || (info *= "  |M|=$(round(mag_scalar; sigdigits=6))")
    isnan(ξ_scalar) || (info *= "  ξ=$(round(ξ_scalar; sigdigits=6))")
    ax = Axis(fig[1, 1]; title="Kagome Lattice Observables  ($info)", aspect=DataAspect(),
              backgroundcolor=:white)
    hidedecorations!(ax)
    hidespines!(ax)
...
function _honeycomb_merge_site_xy(i, j, k)
    ax = Float64(j - 1) * sqrt(3) + Float64(i - 1) * sqrt(3) / 2
    ay = -Float64(i - 1) * 1.5
    if k == 1
        return (ax, ay)
    else
        return (ax + sqrt(3) / 2, ay - 0.5)
    end
end
...
function _bond_offsets_honeycomb_merge(bond_type::String)
    if occursin("J1_onsite", bond_type)
        return 1, 2, (0, 0), (0, 0)
    elseif occursin("J1H", bond_type)
        return 2, 1, (0, 0), (0, 1)
    elseif occursin("J1V", bond_type)
        return 2, 1, (0, 0), (1, 0)
    elseif occursin("J2H2", bond_type)
        return 2, 2, (0, 0), (0, 1)
    elseif occursin("J2V2", bond_type)
        return 2, 2, (0, 0), (1, 0)
    elseif occursin("J2/2", bond_type)
        return 2, 2, (0, 1), (1, 0)
...
function _draw_honeycomb_merge_bonds!(ax, all_coords, e_dict, pattern, unique_sites,
                                      Ni, Nj, n_repeat, e_min, e_max)
    pval_positions = Dict{Int, Vector{Tuple{Int,Int}}}()
    for ci in 1:Ni, cj in 1:Nj
        pv = pattern[ci, cj]
    ...
    for (bond_type, bond_data) in e_dict
        color = _bond_color(bond_type)
        k1, k2, (off1i, off1j), (off2i, off2j) = _bond_offsets_honeycomb_merge(bond_type)
        for (pos_str, eval) in bond_data
            parts = split(pos_str, ",")
            oi, oj = parse(Int, parts[1]), parse(Int, parts[2])
            pv = pattern[oi, oj]
