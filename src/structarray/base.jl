...
# Returns (i,j) for each unique site in ascending site-number order.
# Allows `for (i,j) in eachindex(S)` just like a regular matrix.
Base.eachindex(S::StructArray) =
    map(sort!(unique(vec(S.pattern)))) do q
        Tuple(findfirst(==(q), S.pattern))
    end