...
function ChainRulesCore.rrule(config::RuleConfig, ::typeof(simple_eig_linear_ad), f, x₀; power_iter, ifvalue=false)
    (vals, vecs) = simple_eig(f, x₀; power_iter, ifvalue)
    info = eigsolve(ones(1,1))[3] # dummy info
    howmany = 1
    which = :LM
    ...
    return (vals, vecs), eigsolve_pullback
end
