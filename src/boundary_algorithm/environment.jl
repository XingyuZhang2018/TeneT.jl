...
Array(rt::Tuple{CTMEnv,CTMEnv})    = Array.(rt)
CuArray(rt::Tuple{CTMEnv,CTMEnv})  = CuArray.(rt)
ROCArray(rt::Tuple{CTMEnv,CTMEnv}) = ROCArray.(rt)
...
function update!(env::Tuple, env′::Tuple)
    update!(env[1], env′[1])
    update!(env[2], env′[2])
    return env
end
