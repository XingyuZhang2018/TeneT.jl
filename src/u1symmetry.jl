...
distribute by mod(parts)
average dims of different part dims of U1 tensor bulk
"""
function u1bulkdims(a::Int...; parts::Int = 3)
    average_dims = floor.(Int, a./parts)
    first = a .% parts
    [[[average_dims[i] + 1 for _ in 1:first[i]]; [average_dims[i] for _ in 1:(parts-first[i])]] for i in 1:length(a)]
end

function randU1(atype, dtype, a...)
    L = length(a)
    bkdims = u1bulkdims(a...) # custom initial
    pn = Vector{Vector{Int}}()
    tensor = Vector{atype{dtype}}()
    parts = length(bkdims[1])
    pnmax = floor(Int, parts/2)
    @inbounds for i in CartesianIndices(Tuple(-pnmax:pnmax for _=1:L))
        if sum(i.I) == 0
            push!(pn, collect(i.I))
            dims = Tuple(bkdims[j][i.I[j]+pnmax+1] for j in 1:L)
...
function zerosU1(atype, dtype, a...)
    L = length(a)
    bkdims = u1bulkdims(a...) # custom initial
    pn = Vector{Vector{Int}}()
    tensor = Vector{atype{dtype}}()
...
function IU1(atype, dtype, D)
    bkdims = u1bulkdims(D, D) # custom initial
    pn = Vector{Vector{Int}}()
    tensor = Vector{atype{dtype}}()
    parts = length(bkdims[1])
    pnmax = floor(Int, parts/2)
    @inbounds for i in CartesianIndices(Tuple(-pnmax:pnmax for _=1:2))
        if sum(i.I) == 0
            push!(pn, collect(i.I))
            dims = Tuple(bkdims[j][i.I[j]+pnmax+1] for j in 1:2)
...
function U1bitselection(maxN::Int; parts = 3)
    q = [i % parts for i = 1:maxN]
    return [(q .== (i % parts)) for i in 1:parts]
end
...
function U1tensor2tensor(A::U1tensor{T,N}) where {T,N}
    ...
    pn = A.pn
    qlist = [U1bitselection(size(A)[i]) for i = 1:N]
    shift = Int((length(qlist[1]) + 1)/2)
...
"""
function getpn(L::Int; parts = 3)
    p = Vector{Vector{Int}}()
    pnmax = floor(Int, parts/2)
    for i in CartesianIndices(Tuple(-pnmax:pnmax for i=1:L))
        sum(i.I) == 0 && push!(p, collect(i.I))
    end
    p
end
...
function tensor2U1tensor(A::AbstractArray{T,N}) where {T,N}
    atype = _arraytype(A)
    Aarray = Array(A)
    qlist = [U1bitselection(size(A)[i]) for i = 1:N]
    pn = getpn(N)
...
function tensorpermute(A::AbstractU1Array, perm)
    ...
    U1tensor(A.pn, tensor, A.size[collect(perm)], dims, A.division)
end
...
# """
# function *(A::AbstractU1Array{TA,NA}, B::AbstractU1Array{TB,NB}) where {TA,TB,NA,NB}
#     pn = Vector{Vector{Int}}()
#     dims = Vector{Vector{Int}}()
...
#     ind_A = findall(x->sum(x[divA+1:end]) % 2 == p, Apn)
