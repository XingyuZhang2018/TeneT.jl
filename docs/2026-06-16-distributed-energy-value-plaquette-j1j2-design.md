...
energy **block-distributed**, calling the cannon transfer maps (`FLmap_cannon_dist` etc.)
...
user's explicit request ("energy_value 也需要像之前那样直接调用 FLmap_cannon 来并行,先把
...
| bond | `energy_value` call | kernel chain (observable.jl) | cannon replacement |
|------|--------------------|------------------------------|--------------------|
| J1H  | `contract_*_12`    | `oc_12` = 2× `FLmap_parallel` + `dot(conj(l),FRo)` | `FLmap_cannon_dist` ×2 + `cannon_dot` |
| J1V  | `contract_*_21`    | `oc_21` = 2× `ACmap_parallel` + `dot(conj(u),ACd)` | `ACmap_cannon_dist` ×2 + `cannon_dot` |
| J2\\ | `contract_*_22`    | `oc_22` = `oc_Q_22` + **`qrpos`(full χ)** + `dot(Q,QQ)` | see §oc_22 |
| diag (imag) | `contract_*_11` | `oc_11` = 1× `FLmap_parallel` + `dot(conj(l),FRo)` | `FLmap_cannon_dist` + `cannon_dot` |

`oc_Q_22` = `FLmap` → `ACdmap` → `FRmap` → `ACmap` (4-map chain, observable.jl:99-105). All
four have `*_cannon_dist` variants (cannon_2d.jl 528/740/827/920), and every one already
accepts a `(Au,Ad)` tuple as `M` via `M1,M2 = M isa Tuple ? M : (M,conj(M))` — exactly the
double-layer A the observables pass. `cannon_dot` (cannon_2d.jl:212) + its rrule
(rules.jl:487) exist. `ALCtoAC_cannon` (cannon.jl:349) exists.
...
1. **oc kernels** `oc_11/oc_12/oc_21/oc_22/oc_Q_22` — add `grid = nothing` kwarg. Inside,
   `grid === nothing` keeps `FLmap_parallel`/`dot`; else `FLmap_cannon_dist`/`cannon_dot`.
...
4. **callers** `energy_value(::PlaquetteVUMPSEnv)` and `imag_error(::PlaquetteVUMPSEnv)` —
   `AC = grid === nothing ? ALCtoAC(AL,C) : ALCtoAC_cannon(AL,C,grid)`. (`imag_error` calls
...
`ObsEnv(::PlaquetteVUMPSRuntime)` cannon path currently computes the block obs env then
...
Block-tiling invariant: `FLo`,`FLu` come from `leftenv_cannon` (standard convention: first χ
leg by r1, last by r2). `AL` from the runtime (same). `C` replicated. `AC = ALCtoAC_cannon`
gathers AL → serial `ALCtoAC` → scatters → standard block convention. So every χ-tensor fed
to a cannon map / `cannon_dot` shares the identical disjoint tiling → `cannon_dot` is a single
`+`-allreduce of local pieces (cannon_2d.jl:213), exactly right.

## oc_22 — the one non-trivial kernel

```
Q0   = rand block (χ,D1,D2,χ)         # Zygote.@ignore, see "rank-consistent Q0"
Q1   = oc_Q_22_cannon(Q0, env_blocks…, grid)        # 4 cannon maps, block→block
Q1f  = cannon_gather(Q1, grid)                       # full χ (replicated)
Q,_  = qrpos(reshape(Q1f, χ*D1*D2, χ)); Q = reshape(Q,χ,D1,D2,χ)   # serial, replicated
Qb   = cannon_scatter(Q, grid)                       # back to block
QQ   = oc_Q_22_cannon(Qb, env_blocks…, grid)
return cannon_dot(Qb, QQ, grid)                      # note: dot(Q,QQ), NOT dot(conj…)
```

The `gather → serial full-χ factorization → scatter` seam mirrors `ACCtoALAR_cannon`
(cannon.jl:370-374) exactly — a known, AD-correct pattern (gather take-my-block adjoint →
qrpos rrule replicated → scatter allreduce adjoint). `cannon_gather`/`cannon_scatter` are the
...
- Cannon inherits the same ~1e-10 floor (its own Q0 realization), exactly as a serial re-run
...
All cannon pieces are AD-complete:
- `FLmap/ACmap/FRmap/ACdmap_cannon_dist` — rrules (M3/M4).
- `cannon_dot` — rrule (rules.jl:487): `dx=y·conj(d), dy=x·d`, **no cotangent allreduce**
  (consumed scalar). The observables consume the scalar exactly this way → correct.
- oc_22 seam: `cannon_gather` (take-my-block adj) → `qrpos` rrule → `cannon_scatter`
...
pre-existing in serial; cannon's only added requirement is that each (re)generation stays
...
energy_value (grid=nothing) vs cannon energy_value (grid set), bond-by-bond from `e_dict`:
...
- **total energy** |E_cannon − E_serial| ≤ 1e-8, and physical value reproduces the history
  −0.4713509925 to ≤ 1e-7 (the end-to-end check: cannon leading_boundary + block ObsEnv +
...
- `src/contraction/observable.jl` — `grid=nothing` on oc_11/12/21/22, oc_Q_22, and the
  contract_{n,o}_{11,12,21,22…} wrappers; cannon branches.
- `src/models/basic_interactions.jl` — `_contract_barebones`/`_contract_one` (params) read &
  forward `params.boundary_alg.grid`.
- `src/models/J1J2/energy.jl` — `energy_value(J1J2{Square}, ::PlaquetteVUMPSEnv)`: cannon AC.
- `src/boundary_algorithm/vumps/plaquette.jl` — `imag_error(::PlaquetteVUMPSEnv)` cannon AC +
  grid; `ObsEnv(::PlaquetteVUMPSRuntime)` returns block.
- `src/boundary_algorithm/vumps/cannon.jl` — oc_22 qrpos seam helper if needed; Q0 Bcast.
- `test/test_cannon_energy_plaq.jl` (+ runner) — the parity gate.

## Risks / adversarial-review targets

1. **cannon_dot conj convention** — oc_11/12/21 use `dot(conj(l),FRo)`; oc_22 uses
   `dot(Q,QQ)`. Must pass the matching args to `cannon_dot` (it computes Σconj(x)·y). Verify
...
3. **block tiling match for cannon_dot** — `l`/`u`/`Q` (map outputs) vs the FRo/ACd/QQ they
...
5. **imag_error inside ignore_derivatives** — no AD, but still does MPI collectives
   (cannon_dot/maps) → must be rank-uniform (it is; called on every rank).
6. **double gather** — ALCtoAC_cannon gathers AL, and each cannon map gathers rows/cols
...
- **[BLOCKER 1] grid leak via the shared `_contract_*` params overload.** During a *General*
  cannon run `params.boundary_alg.grid` is also set; if the shared
  ...
  to cannon maps on full tensors → wrong/deadlock. **Fix:** do NOT touch basic_interactions.jl
...
- **[BLOCKER 2] model-blind `ObsEnv` block-return breaks Honeycomb-Plaquette.**
  ...
  (uses `oc_13`, NOT cannon-ized) and Heisenberg Plaquette. **Fix:** `ObsEnv` already receives
  ...
  ObsEnv / energy_value / imag_error so "env is block" ⟺ "use cannon contractions" everywhere.
...
- **[SHOULD-FIX 3] pin cannon_dot conj per site.** `cannon_dot(x,y)=Σconj(x)·y`. Serial
  `dot(conj(l),FRo)=Σl·FRo` ⇒ `cannon_dot(conj(l),FRo,grid)`. `oc_21`'s `dot(conj(u),ACd)` ⇒
  `cannon_dot(conj(u),ACd,grid)`. `oc_22`'s `dot(Q,QQ)=Σconj(Q)·QQ` ⇒ `cannon_dot(Q,QQ,grid)`
  (no extra conj). conj is AD-safe.

- **[SHOULD-FIX 4] oc_22 seam wording.** Same *pattern* as ACCtoALAR_cannon but single-tensor
  `cannon_gather`/`cannon_scatter` (NOT `*_struct`; Q is one tensor). Adjoints: gather
...
- **[SHOULD-FIX 5] Q0 distribution.** Generate full Q0 on rank 0, `MPI.Bcast` over `grid.comm`,
  then `cannon_scatter(Q0_full, grid)` to block (reuse the audited slicer, don't hand-roll the
  r1/r2 partition). All inside `Zygote.@ignore`.

- **[SHOULD-FIX 6] imag_error** must also use `ALCtoAC_cannon` + pass `grid`, gated by the same
