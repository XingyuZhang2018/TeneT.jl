...
## Task 2: Slice2DGrid + test launcher

**Files:**
- Create: `src/contraction/slice2d.jl`
- Modify: `src/TeneT.jl` (add include after line 62)
- Create: `test/test_slice2d.jl`
- Create: `test/run_test_slice2d.jl`

**Step 1: Write the launcher** `test/run_test_slice2d.jl`:

```julia
# Launch test_slice2d.jl under 4 MPI ranks. Usage: julia --project=. test/run_test_slice2d.jl
using MPI
const julia_exe = first(Base.julia_cmd().exec)
const proj = dirname(Base.active_project())
run(`$(MPI.mpiexec()) -n 4 $julia_exe --project=$proj $(joinpath(@__DIR__, "test_slice2d.jl"))`)
```

**Step 2: Write the failing test** `test/test_slice2d.jl`:
...
using Zygote
using TeneT
using TeneT: slice2d_grid, Slice2DGrid, slice2d_scatter, slice2d_gather, FLmap_slice2d,
             FLmap, split_ranges, _slice2d_stage1, _slice2d_stage2
...
Run: `julia --project=. test/run_test_slice2d.jl`
Expected: FAIL — `UndefVarError` (`slice2d_grid` not defined / not importable).

**Step 4: Implement** — create `src/contraction/slice2d.jl`:
...
**Step 6: Run test to verify it passes** (the later imports `slice2d_scatter` etc. will still fail — temporarily comment the not-yet-existing names out of the `using TeneT:` line, or define the testset import minimally; simplest is to import only what exists so far and extend the import line per task)

Run: `julia --project=. test/run_test_slice2d.jl`
Expected: `Test Summary: slice2d_grid | Pass 24` (6 tests × 4 ranks print per-rank summaries) and 4× `rank N: test_slice2d.jl done`.

**Step 7: Commit**

```powershell
git add src/contraction/slice2d.jl src/TeneT.jl test/test_slice2d.jl test/run_test_slice2d.jl
git commit -m "feat: add Slice2DGrid 2D process grid for distributed FLmap"
```
...
**Files:**
- Modify: `src/contraction/slice2d.jl`
- Modify: `test/test_slice2d.jl`

**Step 1: Append the failing test** to `test/test_slice2d.jl` (and add `_slice2d_stage1`, `_slice2d_stage1_add!`, `_slice2d_fold`, `_slice2d_stage2` to the import line):
...
Run: `julia --project=. test/run_test_slice2d.jl`
Expected: FAIL — `_slice2d_stage1` not defined.

**Step 3: Implement** — append to `src/contraction/slice2d.jl`:
...
```powershell
git add src/contraction/slice2d.jl test/test_slice2d.jl
git commit -m "feat: add two-stage Slice2D contraction kernels for leg5 FLmap"
```

---

## Task 4: slice2d_scatter / slice2d_gather (forward)

**Files:**
- Modify: `src/contraction/slice2d.jl`
- Modify: `test/test_slice2d.jl`

**Step 1: Append the failing test** (add `slice2d_scatter`, `slice2d_gather` to imports):
...
**Step 2: Run to verify it fails** — `slice2d_scatter` not defined.
...
```powershell
git add src/contraction/slice2d.jl test/test_slice2d.jl
git commit -m "feat: add slice2d_scatter/slice2d_gather boundary shims"
```

---

## Task 5: FLmap_slice2d forward

**Files:**
- Modify: `src/contraction/slice2d.jl`
- Modify: `test/test_slice2d.jl`

**Step 1: Append the failing test** (add `FLmap_slice2d` to imports):
...
**Step 2: Run to verify it fails** — `FLmap_slice2d` not defined.
...
function _slice2d_row_shift(cur, grid::Slice2DGrid, recv_size; tag = _TAG_BASE + 700)
...
# Shared by FLmap_slice2d and its rrule. Returns (result_blk, H); the rrule
# captures the pre-fold H for the composite fold∘stage2 pullback.
function _slice2d_forward(FL_blk, ALu, ALd, M1, M2, grid::Slice2DGrid)
    N1, N2, r1, r2 = grid.N1, grid.N2, grid.r1, grid.r2
    χ = size(ALu, 1)
    a_rs = split_ranges(χ, N1)
    i_rs = split_ranges(χ, N2)
    l_rng = i_rs[r2 + 1]
    @assert size(FL_blk, 1) == length(a_rs[r1 + 1]) && size(FL_blk, 4) == length(l_rng) "FLmap_slice2d: block shape $(size(FL_blk)) inconsistent with grid ($(N1)×$(N2)) and χ=$χ"
    ...
    for k in 0:N2-1
        ...
        _slice2d_stage1_add!(H, cur, ALd_slice)
        if k < N2 - 1
            t_next = mod(r2 + k + 1, N2)
            cur = _slice2d_row_shift(cur, grid,
    ...
    partial = _slice2d_stage2(G, ALu_slice)
    result = _slice2d_col_reduce_scatter(partial, grid, a_rs)
    return result, H
end

"""
    FLmap_slice2d(FL_blk, ALu, ALd, M, grid; inner_etype=nothing) -> result_blk
...
replicated on every rank. See docs/2026-06-10-slice2d-flmap-design.md.
"""
function FLmap_slice2d(FL_blk, ALu, ALd, M, grid::Slice2DGrid; inner_etype = nothing)
    ...
    result, _ = _slice2d_forward(FL_blk, ALu, ALd, M1, M2, grid)
...
```powershell
git add src/contraction/slice2d.jl test/test_slice2d.jl
git commit -m "feat: FLmap_slice2d forward — two-stage Slice2D ring"
```

---

## Task 6: rrules for slice2d_scatter / slice2d_gather
...
```powershell
git add src/autodiff/rules.jl test/test_slice2d.jl
git commit -m "feat: rrules for slice2d_scatter/slice2d_gather"
```

---

## Task 7: rrule for FLmap_slice2d (the core backward)

**Files:**
- Modify: `src/contraction/slice2d.jl` (add `_slice2d_col_allgather`)
...
**Step 2: Run to verify it fails** — Zygote mutation error inside `_slice2d_forward`.

**Step 3: Implement the column allgather** — append to `src/contraction/slice2d.jl`:
...
function ChainRulesCore.rrule(::typeof(FLmap_slice2d), FL_blk, ALu, ALd, M, grid::Slice2DGrid;
    ...
    result_c, H = _slice2d_forward(FL_c, ALu_c, ALd_c, M1_c, M2_c, grid)
    ...
    function slice2d_back(dresult)
        ...
        _, bp2 = pullback((h, alu, m1, m2) -> _slice2d_stage2(_slice2d_fold(h, m1, m2), alu),
        ...
        for k in 0:N2-1
            ...
            _, bp1 = pullback(_slice2d_stage1, cur, ALd_slice)
            ...
            if N2 > 1
                ...
                if k < N2 - 1   # last replay shift of cur is unnecessary
                    cur = _slice2d_row_shift(cur, grid, sz; tag = _TAG_BASE + 700)
                end
                dacc = _slice2d_row_shift(dacc, grid, sz; tag = _TAG_BASE + 720)
            end
        end
...
```powershell
git add src/contraction/slice2d.jl src/autodiff/rules.jl test/test_slice2d.jl
git commit -m "feat: hand-written rrule for FLmap_slice2d (reverse-replay ring)"
```
...
```powershell
git add test/test_slice2d.jl
git commit -m "test: inner_etype Float32 boundary cast for FLmap_slice2d"
```
...
**Files:**
- Create: `examples/MPI_parallel/test_slice2d_sofia.jl`
- Create: `examples/MPI_parallel/Sofia/submit_test_slice2d.sh`

**Step 1: Write the driver** `examples/MPI_parallel/test_slice2d_sofia.jl`:
...
**Step 2: Write the submit script** `examples/MPI_parallel/Sofia/submit_test_slice2d.sh` (clone of `submit_test_nccl_compare.sh` reduced to 1 node / 4 GPUs; canonical v17 environment):

```bash
#!/bin/bash -l
#SBATCH --job-name=Sofia_slice2d_test
...
# Driver: ../test_slice2d_sofia.jl  Design: docs/2026-06-10-slice2d-flmap-design.md
...
echo "=== Sofia 4-GPU Slice2D FLmap test ==="
echo "Start: $(date)"
...
Run: `julia --project=. -e "include_string(Main, replace(read(\"examples/MPI_parallel/test_slice2d_sofia.jl\", String), r\"^using\"=>\"#using\"; count=1)); println(\"parse ok\")"` — simpler alternative: `julia --project=. -e "Meta.parseall(read(\"examples/MPI_parallel/test_slice2d_sofia.jl\", String)); println(\"parse ok\")"`
Expected: `parse ok`

**Step 4: Commit**

```powershell
git add examples/MPI_parallel/test_slice2d_sofia.jl examples/MPI_parallel/Sofia/submit_test_slice2d.sh
git commit -m "feat: Sofia 4-GPU Slice2D FLmap validation driver + submit script"
```
...
**Files:**
- Modify (results): `docs/2026-06-10-slice2d-flmap-design.md` (append a Results section)
...
**Step 4: Verify output** — `Sofia_slice2d_test_<jobid>.out` must contain:
...
plus `[mem]` and `[time]` lines for both paths. Expected memory picture at map level (be honest in the report): transients are serial/P for **both** paths; Slice2D's map-level saving is the full-vs-block FL input + result (≈ 384 MB/rank at χ=400 D=10); the big wins (eigsolver copies, AD tape) arrive with the leftenv integration round. Timing expectation: slice2d ≤ ~1.5× slice per call at 2×2.

**Step 5: Optional larger size** — resubmit with `TENET_SLICE2D_CHI=800` exported in the script's `BASE_ENVS` to make the full-vs-block difference more visible (FL full = 1.6 GB at χ=800... still H200-comfortable).

**Step 6: Record results** — append a `## Results (2026-06-XX, Sofia 4×H200)` section to `docs/2026-06-10-slice2d-flmap-design.md` with the parity numbers, memory lines, and timing table. Commit:

```powershell
git add docs/2026-06-10-slice2d-flmap-design.md
git commit -m "docs: Sofia 4-GPU Slice2D FLmap validation results"
```
