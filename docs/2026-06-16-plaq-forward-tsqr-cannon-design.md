# Plaquette Cannon Forward TSQR Seam Design

Date: 2026-06-16. Scope: unblock Plaquette observable runs at large chi where the
current Cannon path still performs replicated full-chi QR.

## Goal

Add an opt-in `distributed_qr` path for `VUMPS{<:Plaquette}` on a `CannonGrid`.
...
Sofia job 1287380 failed at chi=2048 during Plaquette Cannon initialization:
`init_VUMPSRuntime_cannon -> left_canonical -> qrpos!`. The same full-chi QR
exists in every Plaquette `ACCtoAL_cannon` step.

## Approach

1. Keep `ACCtoAL_cannon` unchanged.
2. Add `ACCtoAL_tsqr_cannon(AC_blk, C, grid)`.
...
3. Add a Plaquette init path guarded by `distributed_qr=true`.
   ...
   - Build `C` from the TSQR R factors and initialize `FL` as block random data
     before solving `leftenv_cannon`.
4. Route Plaquette `vumps_step_cannon` to TSQR only when `alg.distributed_qr`
   is true.
...
- 4-rank CPU MPI parity: `ACCtoAL_tsqr_cannon` vs serial `ACCtoAL` at small chi.
