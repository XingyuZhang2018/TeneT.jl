# M3: ACmap / ACdmap Cannon cross-axis dataflow (square grid)

Status: design, gating the M3 plan.
Branch: `claude/ecstatic-golick-e3f6f0` (M1+M2 chain engine landed, `CHAIN_ENGINE` default ON).
Template: `FLmap_cannon_dist` rrule (`src/autodiff/rules.jl:589-698`), forward
`_cannon_forward_sliced` (`src/contraction/cannon_2d.jl:417-464`).
...
- **Single-M `dM` composition (was a MAJOR self-contradiction).** Pinned to the
  `FLmap_cannon_dist` precedent (`rules.jl:592,572,690`): the single-M entry
...
`ACmap_cannon_dist` / `ACdmap_cannon_dist` are the cross-axis Cannon maps. The
...
All χ-legged operands follow the Cannon block convention (`cannon_scatter`,
`cannon_2d.jl:164`): **first χ leg split N1-ways by r1, last χ leg split N2-ways
...
- **(i) Gather one free input leg to full before the einsum.** FL carries `i`
  ...
  `_cannon_row_allgather(FL_blk, grid, p_rs)` (existing, tag 750, concatenates the
...
- AC's `d` is on r2 → the N row peers `(r1, 0..N-1)` hold all χ values of `d`
  for the local `a`-block. `_cannon_row_allgather(AC_blk, grid, p_rs)` gives
  `AC[a∈p_rs[r1+1], b, c, d∈1:χ]` — local `a`-block, **full `d`**.
- FR's `d` is on r1 → the N col peers `(0..N-1, r2)` hold all χ values of `d`
  for the local `l`-block. `_cannon_col_allgather(FR_blk, grid, p_rs)` gives
...
`getindex`, no comm) — but note `_cannon_col_reduce_scatter(partial, grid, p_rs)`
...
```
result_blk = _cannon_col_reduce_scatter(partial_{r1}, grid, p_rs)   # tag 710
```

where `partial_{r1}` is `[i∈1:χ, j, k, l∈p_rs[r2+1]]` (full first leg `i`, local
last leg `l`) — exactly the shape `_cannon_col_reduce_scatter` expects (it sums
...
F1. AC_g = _cannon_row_allgather(AC_blk, grid, p_rs)   # full d   (tag 750)  [a-block, full d]
F2. FL_g = _cannon_row_allgather(FL_blk, grid, p_rs)   # full i   (tag 750)  [a-block, full i]
...
F5. result_blk = _cannon_col_reduce_scatter(partial, grid, p_rs)  # sum a over col, keep i-block r1 (tag 710)
...
`a`-partial. Exactly what §2.4 requires. (The cannon rrule always runs this **2M**
`ACMAP_LEG5_CHAIN`; the single-M entry materializes `M2 = conj(M)` at the boundary
and composes `dM = dM1 + conj(dM2)` on exit, pinned to the `FLmap_cannon_dist`
...
`_cannon_col_scatter_first(full, grid, d_rs)`: input `full[a∈1:χ, mid…]`
...
`_cannon_col_allgather(dblk, grid, d_rs)` (existing, tag 730): the cotangent of a
...
concatenation — identical to the `cannon_gather` rrule's "downstream of gather is
...
the adjoint's `_cannon_col_allgather`, which already handles them
(`cannon_2d.jl:275`, N1==1 fast path at line 281, uneven χ via `d_rs` lengths).

The row-axis mirror (`_cannon_row_scatter_last` / adjoint
`_cannon_row_allgather`, tag 750) is defined symmetrically if a free **last** leg
...
would be `_cannon_block_transpose` (NEW tag 770): on a square grid, rank
...
**B5 — adjoint of `_cannon_col_reduce_scatter` (F5).** Reduce-scatter's adjoint is
**allgather** (`cannon_2d.jl:275`, the documented pair). Given `dresult_blk`
(`[i∈p_rs[r1+1], j,k, l∈p_rs[r2+1]]`), produce `dpartial`:

```
dpartial = _cannon_col_allgather(dresult_blk, grid, p_rs)   # [i∈1:χ, j,k, l∈p_rs[r2+1]]  (tag 730)
```
...
`_cannon_col_allgather`. ∎ (Equivalently: reduce-scatter = scatter∘broadcast-sum;
...
`g[3]+g[4]` ≡ `dM1+conj(dM2)`. The cannon rrule nonetheless follows the FLmap
boundary convention `dM1 + conj(dM2)` over the 2M chain, for one consistent code
path with the tuple-M case.)

**B3 — adjoint of `_cannon_col_allgather` on FR (F3).** Allgather's adjoint is
**reduce-scatter** (`cannon_2d.jl:242`, the documented pair). `dFR_g` is full `d`,
local `l`-block; reduce-scatter over col_comm on the first leg `d` returns the
r1-block:

```
dFR_blk = _cannon_col_reduce_scatter(dFR_g, grid, p_rs)    # [d∈p_rs[r1+1], g,h, l∈p_rs[r2+1]]  (tag 710)
```

**B2 — adjoint of `_cannon_row_allgather` on FL (F2).** Row_allgather (last leg
`i`) adjoint is `_cannon_row_reduce_scatter_last` (`cannon_2d.jl:375`, tag 760,
the documented pair). `dFL_g` is local `a`-block, full `i`; reduce-scatter the
LAST leg `i` over row_comm returns the r2-block:

```
dFL_blk = _cannon_row_reduce_scatter_last(dFL_g, grid, p_rs)  # [a∈p_rs[r1+1], e,f, i∈p_rs[r2+1]]  (tag 760)
```

**B1 — adjoint of `_cannon_row_allgather` on AC (F1).** Same primitive: `dAC_g`
is local `a`-block, full `d`; reduce-scatter the LAST leg `d` over row_comm:

```
dAC_blk = _cannon_row_reduce_scatter_last(dAC_g, grid, p_rs)  # [a∈p_rs[r1+1], b,c, d∈p_rs[r2+1]]  (tag 760)
```
...
conj(M))` at entry, exactly as `FLmap_cannon_dist` does at `rules.jl:592`), so B4
...
B5. dpartial = _cannon_col_allgather(dresult, grid, p_rs)                 # tag 730
...
B2. dFL_blk = _cannon_row_reduce_scatter_last(dFL_g, grid, p_rs)          # tag 760
B1. dAC_blk = _cannon_row_reduce_scatter_last(dAC_g, grid, p_rs)          # tag 760
...
(`chain_maps.jl:116`). The cannon rrule does this permutation when wiring B4's
...
## 5. `ACdmap_cannon_dist` — derive forward + backward (SHOW THE WORK)
...
- ACd.i is on r1 → col peers `(0..N-1, r2)` hold all `i` for the local `l`-block.
  `_cannon_col_allgather(ACd_blk, grid, p_rs)` → `ACd[i∈1:χ, j, k, l∈p_rs[r2+1]]`.
- FL.i is on r2 → row peers `(r1, 0..N-1)` hold all `i` for the local `a`-block.
  `_cannon_row_allgather(FL_blk, grid, p_rs)` → `FL[a∈p_rs[r1+1], e, f, i∈1:χ]`.
...
- FR.d on r1 → col peers `(0..N-1, r2)` hold all `d` for the local `l`-block.
  `_cannon_col_allgather(FR_blk, grid, p_rs)` → `FR[d∈1:χ, g, h, l∈p_rs[r2+1]]`.
...
```
result_blk = _cannon_row_scatter_last(partial, grid, p_rs)   # take d∈p_rs[r2+1]; comm-free local slice (§3)
```
...
`getindex` keeping `d∈p_rs[r2+1]`, whose adjoint is `_cannon_row_allgather`
...
```
result_blk = _cannon_row_reduce_scatter_last(partial, grid, p_rs)   # sum l over row, keep d-block r2 (tag 760)
```

Wait — `_cannon_row_reduce_scatter_last` reduces the LAST leg over row_comm. Here
...
row_comm" = `_cannon_row_reduce_scatter_last(partial, grid, p_rs)` (tag 760,
`cannon_2d.jl:375`). **So ACdmap also needs NO new primitive** — it is the
row-axis mirror of ACmap's col_reduce_scatter, and the existing
`_cannon_row_reduce_scatter_last` is exactly the fused op. The "pure free-leg
...
F1. ACd_g = _cannon_col_allgather(ACd_blk, grid, p_rs)   # full i  (tag 730)  [full i, l-block]
F2. FL_g  = _cannon_row_allgather(FL_blk,  grid, p_rs)   # full i  (tag 750)  [a-block, full i]
F3. FR_g  = _cannon_col_allgather(FR_blk,  grid, p_rs)   # full d  (tag 730)  [full d, l-block]
...
F5. result_blk = _cannon_row_reduce_scatter_last(partial, grid, p_rs)  # sum l over row, keep d-block r2 (tag 760)
...
B5. dpartial = _cannon_row_allgather(dresult, grid, p_rs)                 # adjoint of row_reduce_scatter_last; full d (tag 750)
...
B3. dFR_blk  = _cannon_col_reduce_scatter(dFR_g, grid, p_rs)              # adjoint of F3 col_allgather (tag 710)
B2. dFL_blk  = _cannon_row_reduce_scatter_last(dFL_g, grid, p_rs)        # adjoint of F2 row_allgather (tag 760)
B1. dACd_blk = _cannon_col_reduce_scatter(dACd_g, grid, p_rs)            # adjoint of F1 col_allgather (tag 710)
BM. allreduce_p2p!(dM1,+,comm); allreduce_p2p!(dM2,+,comm)
...
Adjoint pairs used (all existing, all documented in `cannon_2d.jl`):
...
### 6.1 Signatures (mirror `FLmap_cannon_dist`)
...
`FLmap_cannon_dist`, `rules.jl:592,572,690`):** at entry set
...
the cannon rrule follows the FLmap 2M-chain + boundary-compose path for one
...
byte-for-byte the FLmap_cannon_dist boundary-cast block (`rules.jl:594-607,
...
`ACdmap_cannon_dist` derives `n_i = n_d = N·⌈√forloop_iter⌉` internally from
...
Implementation choice mirroring FLmap_cannon_dist: do the three gathers **once**
...
by the non-AD and rrule paths (the FLmap `_cannon_forward_sliced` pattern). In an
iteration with fixed FL/FR (e.g. the ACenv power loop), hoisting the gathers out
of the loop is the natural optimization, as noted for FLmap_cannon_dist.
...
the map rrule or inside any map's chain backward** (MEMORY: cannon+chain engine).

---

## 8. Test strategy (2×2 grid, 4 ranks, parity vs serial)

Extend `test/test_cannon.jl` (4-rank harness, CPU `Array`, `@assert
Comm_size==4`). All comparisons via `cannon_scatter`/`cannon_gather` (+ their
...
```julia
using TeneT: ACmap, ACdmap, ACmap_cannon_dist, ACdmap_cannon_dist
make_leg5 reused (FL, ALu→FL/FR roles, AC/ACd via rand(ComplexF64, χ, D, D, χ))

# Forward parity (1e-12):
ref  = ACmap(AC, FL, FR, (M1, M2))
out  = cannon_gather(ACmap_cannon_dist(
          cannon_scatter(AC,g), cannon_scatter(FL,g), cannon_scatter(FR,g), (M1,M2), g; forloop_iter=n), g)
...
ACdmap test is identical with `ACdmap`/`ACdmap_cannon_dist`, output convention
...
back as input: `ACmap_cannon_dist(out_blk, FLb, FRb, …)` matches the serial
...
`ACmap_cannon_dist` and compares the composed serial map.

GPU coverage: extend `examples/MPI_parallel/test_cannon_sofia.jl` (the
...
4. **No transpose primitive lands in v1.** §2.6/§5 prove v1 fuses every cross-axis
   ...
   no matching reduction to fuse with), implement `_cannon_block_transpose`
...
5. **`partial` first-leg fullness assumption in col_reduce_scatter (ACmap F5).**
   `_cannon_col_reduce_scatter` extracts `partial[d_rs[j+1],…]` per destination
   (`cannon_2d.jl:245`). ACmap's `partial` first leg is **full `i`**, so the
...
6. **Rectangular grid (`N1≠N2`) is genuinely different**, not a parameter tweak:
   ...
   v2 needs the real `_cannon_block_transpose` with `alltoallv` per-(src,dst)
...
7. **Single-M composition — PINNED to the FLmap precedent (no ambiguity).** The
   cannon rrule routes the single-M case through the **2M tuple chain
...
- **ACdmap memory (was BLOCKER).** Root cause: `tensor_pinned_inters` on
  ...
  materialized.
- **Single-M `dM` (was MAJOR).** Pinned to the `FLmap_cannon_dist` precedent:
