# M3: Cannon distributed map wrappers (Cmap / FRmap / ACmap / ACdmap)

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to
> implement this plan task-by-task, with a double review (spec compliance +
> code quality) after every task.

**Goal:** Four new distributed cross-/co-axis Cannon maps —
`Cmap_cannon`, `FRmap_cannon_dist`, `ACmap_cannon_dist`,
`ACdmap_cannon_dist` — each with a forward, a Zygote-free `rrule`, and a
4-rank (2×2) MPI distributed-parity gate (forward rtol 1e-12, gradient rtol
1e-10) vs the serial kernel. M3 is **additive**: `FLmap_cannon_dist`,
`FLmap_cannon`, the serial `basic.jl` kernels, and the M2 chain engine stay
...
entirely from the existing `cannon_2d.jl` primitives and their documented
...
`FLmap_cannon_dist` (`src/contraction/cannon_2d.jl:516`, rrule
...
- **Cross-axis (gather) class — FRmap, ACmap, ACdmap.** Each has **two**
  ...
  simultaneously; the design doc (`docs/2026-06-13-acmap-cannon-dataflow-design.md`)
...
NEVER inside any rrule backward; cannon-OOM lesson), CUDA.jl (`_free!`),
TensorOperations 5.5.1 runtime API via the chain engine. Existing
`cannon_2d.jl` comm primitives + `chain_maps.jl` chains as the building
blocks.
...
M3 writes **NO new `_cannon_<map>_*` hand kernels.** The M2 chain engine is
...
`CMAP_LEG*_CHAIN`). **No map uses the cannon-ring API** (`chain_link1_add!` /
...
B). The ring APIs and the M1 *"chain engine reproduces the cannon local
pipeline"* testset are not part of M3 at all.

The ONLY hand-written code is the comm wrapper, and every comm op already
exists in `cannon_2d.jl` with a documented adjoint — **no new comm primitive
lands in M3** (design doc §2.6, §5, risk 4). The four existing primitives M3
needs are: `_cannon_col_allgather` (730), `_cannon_row_allgather` (750),
`_cannon_col_reduce_scatter` (710), `_cannon_row_reduce_scatter_last` (760),
...
| What | Where |
|------|-------|
| `FLmap_cannon_dist` forward (signature template) | `src/contraction/cannon_2d.jl:516-535` |
| `_cannon_forward_sliced` (gather + chunk core) | `src/contraction/cannon_2d.jl:417-464` |
| `rrule(FLmap_cannon_dist)` (rrule STRUCTURAL template) | `src/autodiff/rules.jl:589-698` |
| do_cast / densify / NoTangent / eager `_free!` boundary | `src/autodiff/rules.jl:591-607,614-623,690-695` |
| `CannonGrid`, `cannon_grid`, `cannon_scatter`/`gather` | `src/contraction/cannon_2d.jl:16-50,164-203` |
| `cannon_gather` rrule (take-my-block adjoint — Cmap template) | `src/autodiff/rules.jl:450-461` |
| `cannon_scatter` rrule (allreduce-stitch adjoint) | `src/autodiff/rules.jl:~430-447` |
| `_cannon_col_reduce_scatter` (first leg, tag 710) | `src/contraction/cannon_2d.jl:242` |
| `_cannon_col_allgather` (first leg, tag 730, adjoint of 710) | `src/contraction/cannon_2d.jl:275` |
| `_cannon_row_allgather` (last leg, tag 750) | `src/contraction/cannon_2d.jl:339` |
| `_cannon_row_reduce_scatter_last` (last leg, tag 760, adjoint of 750) | `src/contraction/cannon_2d.jl:375` |
| `_free!` (CuArray unsafe_free) | `src/contraction/cannon_2d.jl:71-72` |
...
| 4-rank parity harness (CLONE this) | `test/test_cannon.jl`, launcher `test/run_test_cannon.jl` |
| Sofia 4-GPU cannon driver + submit (Batch E template) | `examples/MPI_parallel/test_cannon_sofia.jl`, `examples/MPI_parallel/Sofia/submit_test_cannon.sh` |
| Design doc (AUTHORITATIVE for AC/ACdmap) | `docs/2026-06-13-acmap-cannon-dataflow-design.md` |
...
Cannon convention: first χ leg by r1, last χ leg by r2. Per-leg census for
...
F1. ARd_g = _cannon_col_allgather(ARd_blk, grid, p_rs)   # full i   (tag 730)  [full i, l-block]
F2. ARu_g = _cannon_row_allgather(ARu_blk, grid, p_rs)   # full d   (tag 750)  [a-block, full d]
F3. FR_g  = _cannon_col_allgather(FR_blk,  grid, p_rs)   # full d   (tag 730)  [full d, l-block]
...
F5. result_blk = _cannon_row_reduce_scatter_last(partial, grid, p_rs)  # sum l over row, keep i-block r2 (tag 760)
...
`FRmap_cannon_dist` / `ACmap_cannon_dist` / `ACdmap_cannon_dist` **assume
...
julia --project=. test/run_test_cannon.jl        # existing 4-rank FLmap suite (must STAY green)
julia --project=. test/run_test_cannon_m3.jl      # NEW — 4-rank M3 maps suite (this plan)
...
Add the four new maps' parity tests to a **new** `test/test_cannon_m3.jl` with
its own launcher `test/run_test_cannon_m3.jl` (clone of
`test/run_test_cannon.jl`, pointing at the new test file). Rationale: keeps the
existing `test_cannon.jl` (FLmap, the M3-untouched gate) byte-stable so a green
`run_test_cannon.jl` stays a clean regression signal, while the new file grows
batch by batch. Both launchers run under `mpiexec -n 4`; the new test file
clones `test_cannon.jl`'s header verbatim (`MPI.Init()`, `comm`/`rank` consts,
...
Cmap_cannon`; Batch B appends `FRmap, FRmap_cannon_dist`; Batch C appends
`ACmap, ACmap_cannon_dist`; Batch D appends `ACdmap, ACdmap_cannon_dist`. The
shared helpers (`cannon_grid`, `CannonGrid`, `cannon_scatter`, `cannon_gather`,
...
- **No Zygote inside any `rrule` backward.** Recompute-style + eager `_free!`,
  exactly the `FLmap_cannon_dist` rrule discipline. Zygote appears only as the
  test parity reference and the outer driver that calls the rrule.
- **Old paths untouched and green.** `basic.jl` serial kernels,
  `FLmap_cannon`, `FLmap_cannon_dist`, the chain engine, `test_cannon.jl`.
...
- **do_cast boundary cast** = `inner_etype !== nothing && inner_etype !=
  ...
  byte-for-byte the FLmap_cannon_dist block.
...
- **NO new comm primitive.** M3 uses ONLY the four existing primitives
  (`_cannon_col_allgather` 730, `_cannon_row_allgather` 750,
  `_cannon_col_reduce_scatter` 710, `_cannon_row_reduce_scatter_last` 760) and
  their documented adjoint partners. No `_cannon_col_shift`, no col-twin
  reduce-scatter, no `_cannon_block_transpose` — all proven unnecessary for v1
  (design doc §2.6/§5/risk 4). No new `_cannon_<map>_*` hand kernel either.
- **Include-order / late-bind note.** `cannon_2d.jl` is included BEFORE
  `chain_engine.jl` / `chain_maps.jl`, so the chain consts
  (`FRMAP_LEG5_CHAIN` etc.) are not yet defined when the `*_cannon_dist`
...
- The 4-rank parity gate (1e-12 fwd / 1e-10 grad) must pass BEFORE a batch is
  done; `run_test_cannon.jl` must stay green throughout.

---

## Batch A — Cmap_cannon (3 tasks)
...
`test/test_cannon_m3.jl` + `test/run_test_cannon_m3.jl` are validated end to
end and every later batch only appends testsets.

### Task A1: Cmap_cannon forward (allgather FL/FR → chain_apply, C replicated)

**Files:** modify `src/contraction/cannon_2d.jl`; create
`test/test_cannon_m3.jl` + `test/run_test_cannon_m3.jl`.

**Step 1 — failing forward test.** Create `test/test_cannon_m3.jl` with the
`test_cannon.jl` header, then a first testset (CPU `Array`, 4 ranks). Import
...
`test/run_test_cannon_m3.jl` (clone of `run_test_cannon.jl`):

```julia
# Launch test_cannon_m3.jl under 4 MPI ranks. Usage: julia --project=. test/run_test_cannon_m3.jl
using MPI
const julia_exe = first(Base.julia_cmd().exec)
const proj = dirname(Base.active_project())
run(`$(MPI.mpiexec()) -n 4 $julia_exe --project=$proj $(joinpath(@__DIR__, "test_cannon_m3.jl"))`)
```

**Step 2 — run, expect FAIL** (`Cmap_cannon` undefined):
`julia --project=. test/run_test_cannon_m3.jl`

**Step 3 — implement** in `cannon_2d.jl` (after the FLmap block). Cmap leg4 is
...
> `_cannon_col_allgather`/`_cannon_row_allgather` compose correctly for a 4-leg
> (leg4) and 3-leg (leg3) tensor — the leg3 Cmap has FL `[a,c,d]` (a=r1, d=r2)
> and FR `[b,c,e]` (b=r1, e=r2).

**Step 4 — run, expect PASS** (m3 + cannon suites). **Step 5 — commit:**
`feat: Cmap_cannon forward (gather FL/FR → chain_apply, C replicated)`

### Task A2: Cmap_cannon rrule (take-my-block FL/FR, dC replicated)

**Files:** modify `src/autodiff/rules.jl`; extend `test/test_cannon_m3.jl`.
...
> is **TAKE-MY-BLOCK** (a `getindex` slice), exactly the `cannon_gather` rrule
...
> `FLmap_cannon_dist`, where reduce-scatter IS correct** — there the loss depends
...
- **B3 — dFL/dFR take-my-block:** the gathers (col_allgather ∘ row_allgather)
  ...
  the full cotangent" — exactly the `cannon_gather` rrule (`rules.jl:455-459`):
...
- return `(NoTangent(), dC, dFL_blk, dFR_blk, NoTangent())` — map order
  `Cmap_cannon(C, FL_blk, FR_blk, grid)`.

**Step 4 — run, expect PASS.** **Step 5 — commit:**
`feat: rrule(Cmap_cannon) (take-my-block FL/FR grads, dC replicated)`

### Task A3: Cmap_cannon leg3 + replicated-C invariant

**Files:** extend `test/test_cannon_m3.jl`.
...
`test: Cmap_cannon leg3 parity + replicated-output invariant`

---

## Batch B — FRmap_cannon_dist (5 tasks)
...
### Task B1: FRmap_cannon_dist forward (F0-F5, 2-level d/i chunk, square grid)

**Files:** modify `src/contraction/cannon_2d.jl`; extend `test/test_cannon_m3.jl`.
...
on r1, i on r2. Append to `test_cannon_m3.jl` (and add `FRmap, FRmap_cannon_dist`
...
**Step 2 — run, expect FAIL** (`FRmap_cannon_dist` undefined).

**Step 3 — implement** F0-F5 (the i↔d swap of ACdmap's §5.1). In `cannon_2d.jl`:
...
**Step 4 — run, expect PASS** (m3 + cannon). The off-diagonal assertion is the
diagonal-trap guard; the `forloop_iter=3` case forces `n_d,n_i ≥ 2N`. **Step 5
— commit:** `feat: FRmap_cannon_dist forward (F0-F5, 2-level d/i chunk, square grid)`

### Task B2: FRmap_cannon_dist rrule (B0-B5, same 2-level chunk)

**Files:** modify `src/autodiff/rules.jl` (clone `rrule(ACdmap_cannon_dist)`
with i↔d swap); extend `test/test_cannon_m3.jl`.
...
- **B0:** densify non-DenseArray cotangent (FillArrays guard, `rules.jl:618-622`);
  do_cast.
- **B5:** `dpartial = _cannon_row_allgather(d_c, grid, p_rs)` (tag 750, adjoint
...
- **B3 — adjoint of F3 col_allgather on FR:** `dFR_blk =
  _cannon_col_reduce_scatter(dFR_g, grid, p_rs)` (tag 710) — `dFR_g` full d,
  local l-block → keep d-block r1.
- **B2 — adjoint of F2 row_allgather on ARu:** `dARu_blk =
  _cannon_row_reduce_scatter_last(dARu_g, grid, p_rs)` (tag 760) — `dARu_g`
  a-block, full d → keep d-block r2.
- **B1 — adjoint of F1 col_allgather on ARd:** `dARd_blk =
  _cannon_col_reduce_scatter(dARd_g, grid, p_rs)` (tag 710) — `dARd_g` full i,
...
- return `(NoTangent(), dFR_blk, dARu_blk, dARd_blk, dM, NoTangent())` — map arg
  order `FRmap_cannon_dist(FR, ARu, ARd, M, grid)`. The chain returns
...
**Step 4 — run, expect PASS** (m3 + cannon). **Step 5 — commit:**
`feat: rrule(FRmap_cannon_dist) (B0-B5, same d/i chunk, chain_backward, no Zygote)`
...
`test: FRmap_cannon_dist forloop_iter clamp parity`

### Task B4: FRmap distributed self-consistency (square-grid gate)

**Files:** none new; explicit **gate confirmation** — run the full
`test/run_test_cannon_m3.jl` and `test/run_test_cannon.jl`, confirm Batch B is
...
**Commit:** `docs: FRmap_cannon_dist square-grid gate (gather-class, not a ring)`
...
## Batch C — ACmap_cannon_dist (5 tasks)

The cross-axis (gather) class. Implement EXACTLY the design doc's ACmap
F1-F5 / B1-B5 (`docs/2026-06-13-acmap-cannon-dataflow-design.md` §2.7, §4.2).
...
### Task C1: ACmap_cannon_dist forward (F0-F5)

**Files:** modify `src/contraction/cannon_2d.jl`; extend `test/test_cannon_m3.jl`.

**Step 1 — failing forward test + the CRITICAL off-diagonal assertion** (design
doc §8). Append `ACmap, ACmap_cannon_dist` to the `using TeneT:` line (NIT —
per-batch imports), then append to `test_cannon_m3.jl`:
...
**Step 3 — implement** F0-F5 verbatim from design doc §2.7. In `cannon_2d.jl`:
...
> `Σ_a`. Confirm `_cannon_row_allgather`/`_cannon_col_allgather` produce the
> shapes the docstrings claim (last-leg concat for row 750, first-leg concat
> for col 730).

**Step 4 — run, expect PASS** (m3 + cannon). The off-diagonal assertion is the
diagonal-trap guard. **Step 5 — commit:**
`feat: ACmap_cannon_dist forward (F0-F5, gather-i, square grid)`

### Task C2: ACmap_cannon_dist rrule (B0-B5)
...
- **B5:** `dpartial = _cannon_col_allgather(d_c, grid, p_rs)` (tag 730) → full
  i, local l-block.
...
- **B3:** `dFR_blk = _cannon_col_reduce_scatter(dFR_g, grid, p_rs)` (tag 710).
- **B2:** `dFL_blk = _cannon_row_reduce_scatter_last(dFL_g, grid, p_rs)` (760).
- **B1:** `dAC_blk = _cannon_row_reduce_scatter_last(dAC_g, grid, p_rs)` (760).
- **BM:** `allreduce_p2p!(dM1,+,grid.comm)`; `allreduce_p2p!(dM2,+,grid.comm)`;
  `dM = is_tuple ? (dM1,dM2) : dM1 .+ conj(dM2)`; do_cast upcast.
- return `(NoTangent(), dAC_blk, dFL_blk, dFR_blk, dM, NoTangent())` — map order
  `ACmap_cannon_dist(AC, FL, FR, M, grid)`.
...
**Step 4 — run, expect PASS** (m3 + cannon). **Step 5 — commit:**
`feat: rrule(ACmap_cannon_dist) (B0-B5, chain_backward, no Zygote)`

### Task C3: ACmap single-M + bare-sum + inner_etype

**Files:** extend `test/test_cannon_m3.jl` with the single-M `dM = dM1 +
...
**Commit:** `test: ACmap_cannon_dist single-M / densify / inner_etype`

### Task C4: ACmap iterability (output feeds back as input)

**Files:** extend `test/test_cannon_m3.jl`. ACmap output `[i,j,k,l]` has i on
...
**Commit:** `test: ACmap_cannon_dist iterability (output block feeds back)`

### Task C5: ACmap square-grid assertion + memory-bound comment

**Files:** review `cannon_2d.jl` ACmap — confirm the `@assert N1==N2`, the
...
`chore: ACmap_cannon_dist square-grid + memory-bound asserts (risks 2,5)`

---

## Batch D — ACdmap_cannon_dist (5 tasks)
...
### Task D1: ACdmap_cannon_dist forward (F0-F5, 2-level i/d chunk)

**Files:** modify `src/contraction/cannon_2d.jl`; extend `test/test_cannon_m3.jl`.

**Step 1 — failing forward test + off-diagonal (a,d) assertion** (design doc
§5.1 completeness, §8). Append `ACdmap, ACdmap_cannon_dist` to the `using TeneT:`
...
# Design: docs/2026-06-13-acmap-cannon-dataflow-design.md §5-§6.
...
function _acdmap_cannon_forward_sliced(ACd_g, FR_g, FL_g, M1, M2, grid, p_rs, n_i, n_d; forloop_iter = 1)
...
function ACdmap_cannon_dist(ACd_blk, FL_blk, FR_blk, M, grid::CannonGrid;
    ...
    FL_g  = _cannon_row_allgather(FL_blk, grid, p_rs)           # F2: full i (tag 750)
    FR_g  = _cannon_col_allgather(FR_blk, grid, p_rs)           # F3: full d (tag 730)
    result, _, _, _ = _acdmap_cannon_forward_sliced(ACd_g, FR_g, FL_g, M1, M2, grid, p_rs, n_chunk, n_chunk; forloop_iter)
    return do_cast ? T_orig.(result) : result
end
```
...
**Step 4 — run, expect PASS** (m3 + cannon). **Step 5 — commit:**
`feat: ACdmap_cannon_dist forward (F0-F5, 2-level i/d chunk, square grid)`

### Task D2: ACdmap_cannon_dist rrule (B0-B5, same 2-level chunk)
...
- **B3:** `dFR_blk = _cannon_col_reduce_scatter(dFR_g, grid, p_rs)` (710,
  adjoint of F3 col_allgather).
- **B2:** `dFL_blk = _cannon_row_reduce_scatter_last(dFL_g, grid, p_rs)` (760,
  adjoint of F2 row_allgather).
- **B1:** `dACd_blk = _cannon_col_reduce_scatter(dACd_g, grid, p_rs)` (710,
...
- return `(NoTangent(), dACd_blk, dFL_blk, dFR_blk, dM, NoTangent())` — map
  order `ACdmap_cannon_dist(ACd, FL, FR, M, grid)`; chain `(dACd,dFR,...,dFL)`
  permutes to `(dACd, dFL, dFR, dM)` (chain_maps.jl:147).

**Step 4 — run, expect PASS** (m3 + cannon). **Step 5 — commit:**
`feat: rrule(ACdmap_cannon_dist) (B0-B5, same i/d chunk, chain_backward, no Zygote)`

### Task D3: ACdmap single-M + densify + inner_etype + non-uniform-bond (Db≠Dc)

**Files:** extend `test/test_cannon_m3.jl` — single-M `dM` composition,
...
**Commit:** `test: ACdmap_cannon_dist single-M / densify / inner_etype / Db≠Dc dim guard`

### Task D4: ACdmap not-self-iterating (compose into ACmap)

**Files:** extend `test/test_cannon_m3.jl`. ACdmap is NOT self-iterating
(output is `{a,d}` top, input is `{i,l}` bottom — design doc §8). Its iterate
test feeds its output into a matching `ACmap_cannon_dist` and compares the
...
**Commit:** `test: ACdmap_cannon_dist composes into ACmap (iterate analog)`

### Task D5: ACdmap chunk-leg + memory-bound audit

**Files:** review `cannon_2d.jl` ACdmap — confirm the inline comment block
...
`chore: ACdmap_cannon_dist chunk-leg + memory-bound audit (risks 1,3)`
...
by the user / `/hpc` flow). Clone the existing Sofia cannon pattern
(`examples/MPI_parallel/test_cannon_sofia.jl` +
`examples/MPI_parallel/Sofia/submit_test_cannon.sh`; see
`examples/MPI_parallel/benchmarks/Sofia_VUB_H200.md` Part 5).

### Task E1: Sofia M3 validation driver

**Files:** create `examples/MPI_parallel/test_cannon_m3_sofia.jl` (clone
`test_cannon_sofia.jl`).
...
`TENET_CANNON_CHI` default 400, `D` from `TENET_CANNON_D` default 10), compare
...
with the `mem_line` / `timed` helpers verbatim from `test_cannon_sofia.jl:17-31`.
...
> the SAME `*_cannon_dist` code the tests exercise — the GPU is the only new
> variable.
...
**Files:** create `examples/MPI_parallel/Sofia/submit_test_cannon_m3.sh` (clone
`submit_test_cannon.sh`). Same module loads, `WD`, `CLEAN_LD`, `BASE_ENVS`,
`mpirun -np 4` invocation; point the driver at `../test_cannon_m3_sofia.jl`;
...
it lands.** **Commit (after run):** `docs: Part 9 — M3 cannon-wrapper 4-GPU Sofia validation`
...
- `Cmap_cannon`, `FRmap_cannon_dist`, `ACmap_cannon_dist`,
  `ACdmap_cannon_dist` each have a forward + a Zygote-free `rrule`.
- `julia --project=. test/run_test_cannon_m3.jl` green: every map's 4-rank
...
- Local einsums route through the chain engine via the **whole-chain API**
  ...
  `_cannon_<map>_*` hand kernels, and NO cannon-ring API** (FRmap is gather-class,
  not a ring).
- **NO new comm primitive** — M3 uses ONLY the four existing primitives
  (`_cannon_col_allgather` 730, `_cannon_row_allgather` 750,
  `_cannon_col_reduce_scatter` 710, `_cannon_row_reduce_scatter_last` 760) and
  their documented adjoint partners. No `_cannon_col_shift`, no col-twin
  reduce-scatter, no `_cannon_block_transpose`.
- Cmap's FL/FR gradients are **take-my-block** (`getindex` slice), NOT
  ...
  with no allreduce. (Contrast FLmap_cannon_dist, whose distributed output makes
...
- NO Zygote inside any rrule backward; eager `_free!` discipline matches the
  ...
  χ×χ plane.
- `julia --project=. test/run_test_cannon.jl`, `test/test_chain_engine.jl`,
...
- **Rectangular grids for FRmap/AC/ACdmap** (`N1≠N2`) — M3 v2; needs the real
  `_cannon_block_transpose` (tag 770, design doc §3, risk 6). The square
...
- **Distributed leg4/leg8 variants** of the gather-class maps — only leg5 (tuple
  + single M) is wired (matches the FLmap_cannon_dist scope and the AD census).
...
- A free-leg scatter/allgather primitive (design doc §3) and
  `_cannon_block_transpose` (tag 770) — proven unnecessary for M3 v1; reserved.
