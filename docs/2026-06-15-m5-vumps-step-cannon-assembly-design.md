# M5 design — Cannon `vumps_step` assembly + the QR gather seam
...
Assemble the four M4 distributed env solvers into a distributed VUMPS *step* — a cannon `VUMPS{General}` path that runs the full power-method iteration over block-distributed boundary tensors on a square `N1=N2` `CannonGrid`. The deliverable is a cannon `vumps_step` reachable from `leading_boundary → vumps_itr`, whose fixed-point trajectory matches the serial `vumps_step` exactly, and whose gradient matches under Zygote.

**What M4 already gives us** (`src/boundary_algorithm/vumps/cannon.jl`):

| serial call | cannon analog | status |
|---|---|---|
| `leftenv` (general.jl:254) | `leftenv_cannon` (cannon.jl:54) | ✅ M4, parity-gated, 16-GPU |
| `rightenv` (general.jl:355) | `rightenv_cannon` (cannon.jl:135) | ✅ M4 |
| `ACenv` (general.jl:562) | `ACenv_cannon` (cannon.jl:210) | ✅ M4 |
| `Cenv` (general.jl:640) | `Cenv_cannon` (cannon.jl:283) | ✅ M4 (outputs **replicated** C) |

**What M5 must build** — the two pipeline endpoints plus the orchestration:

1. **`ALCtoAC_cannon`** — the *entry* seam (block AL × replicated C → block AC). General.jl:196.
2. **`ACCtoALAR_cannon`** — the *exit* seam, the **QR gather seam**: full-χ per-cell QR/LQ that cannot run on a block. General.jl:712.
3. **`vumps_step_cannon`** — the 6-call orchestration mirroring serial `vumps_step` (general.jl:873).
4. **`init_VUMPSRuntime_cannon`** — build a block-distributed initial runtime.
5. **Dispatch wiring** — route `leading_boundary → vumps_itr → vumps_step` to the cannon path.
...
**Inherited hard constraints** (asserted in all four M4 solvers, e.g. cannon.jl:55-59): square grid `N1=N2`, leg5 MPO only, `ifsimple_eig=true`, no mixed precision (`inner_etype === nothing`), no observable env (`ifobs=false`). The M5 step asserts the same regime; the gate runs under exactly it.

---

## 1. The data flow — what is block vs replicated at every point

Serial `vumps_step` (general.jl:873-886), annotated with the cannon storage class of each tensor:

```julia
function vumps_step_cannon(rt, M, grid, alg)
    @unpack AL, C, AR, FL, FR = rt          # AL,AR,FL,FR = BLOCK ;  C = REPLICATED (full χ×χ)
    sub = alg.subop_checkpoint
    AC = ALCtoAC_cannon(AL, C, grid)                                            # 1  block AL × repl C → BLOCK AC
    _, FL = checkpoint(sub, (a,b,m,fl)->leftenv_cannon(a,b,m,fl,grid; alg), AL, conj(AL), M, FL)    # 2  → BLOCK FL
    _, FR = checkpoint(sub, (a,b,m,fr)->rightenv_cannon(a,b,m,fr,grid; alg), AR, conj(AR), M, FR)   # 3  → BLOCK FR
    _, AC = checkpoint(sub, (ac,fl,m,fr)->ACenv_cannon(ac,fl,m,fr,grid; alg), AC, FL, M, FR)        # 4  → BLOCK AC
    _, C  = Cenv_cannon(C, FL, FR, grid; alg)                                   # 5  → REPLICATED C (no checkpoint, serial parity)
    AL, AR, errL, errR = checkpoint(sub, (ac,c)->ACCtoALAR_cannon(ac,c,grid), AC, C)               # 6  QR seam → BLOCK AL,AR
...
- `AL, AR, FL, FR` are **block** StructArrays throughout the steady state. First χ leg split by `r1`, last χ leg split by `r2` (`cannon_scatter` convention, cannon_2d.jl:164-171).
- `C` is **replicated** (full χ×χ on every rank) throughout. `Cenv_cannon` produces replicated C (cannon.jl:313); the QR seam consumes replicated C without gathering it; `LRtoC` at init produces replicated C.
- `M` is **replicated** leg5.

**Parity-critical ordering subtlety** (research, vumps-step report §1): `vumps_step` feeds the **OLD** `AL`/`AR` (from `rt`) into `leftenv`/`rightenv` — *not* the freshly-QR'd ones. There is a single AC/C solve per step. (Contrast `vumps_step_power`, general.jl:856, which re-solves and feeds new `ALp`/`ARp`.) The cannon step must replicate this exact ordering or the trajectory diverges from serial. We mirror `vumps_step`, not `vumps_step_power`.

---

## 2. The QR gather seam — `ACCtoALAR_cannon` (the genuinely new differentiable region)
...
`C` is **already replicated** full χ×χ (M4 `Cenv_cannon`), so `C[i]`, `RC`/`LC`/`QC` need **no** gather. Only **AC** is gathered.
...
```julia
function ACCtoALAR_cannon(AC_blk, C, grid)
...
where `gather_struct`/`scatter_struct` map `cannon_gather`/`cannon_scatter` over `.data`, built inside a `Zygote.Buffer` loop mirroring `Cenv_cannon`'s buffer pattern (cannon.jl:287-313) so they are differentiable. The loop runs over `S.data` (the **unique** cells) — rank-uniform because `.pattern` is replicated.
...
This mirrors `Cenv_cannon` (cannon.jl:283) — gather operands to full, run the **unmodified serial kernel** — except (unlike Cenv, which keeps C replicated) the seam re-scatters the block AL/AR outputs. `cannon_gather` (cannon_2d.jl:179) and `cannon_scatter` (cannon_2d.jl:164) are pure data-movement (gather = allgatherv, scatter = local indexing).

### 2.3 The adjoint — why it composes correctly, and the over-count trap

The whole point: **we write zero new rrules.** The seam is a composition of `cannon_gather`, the serial QR (`qrpos`/`lqpos` rrules, rules.jl:64-92), and `cannon_scatter` — each already has a correct rrule, and Zygote composes them.

Backward chain (Zygote replays in reverse):

| forward op | rrule | adjoint action |
|---|---|---|
| `cannon_scatter(AL_full)` | rules.jl:448 | **allreduce** the block AL cotangents into the full replicated `dAL_full` |
| serial QR `ACCtoAL/AR(AC_full,C)` | rules.jl:64-92 | `qrpos`/`lqpos` pullback on full χ — **runs identically on every rank** |
| `cannon_gather(AC_blk)` | rules.jl:466 | **take-my-block**: each rank slices its own block of the replicated `dAC_full` |

Why each is correct:

- **`cannon_gather` adjoint = take-my-block** (rules.jl:475). Justified by the comment there: *"Downstream of gather is replicated computation → identical dfull on every rank; the adjoint is just take-my-block."* The full AC feeds a QR that runs identically on every rank → the cotangent `dAC_full` is identical on every rank → each rank takes only its own block. This is the same case as `Cenv_cannon`'s fixed FL/FR gather (cannon.jl:255-256).
- **`cannon_scatter` adjoint = allreduce** (rules.jl:460). The scatter forward turns a replicated full AL into this rank's block; its adjoint stitches the disjoint per-rank block cotangents into the full replicated-input gradient via allreduce. Exactly right for re-scattering the QR's full AL/AR.
- **QR pullback is replicated** — see §2.4.

**🔴 THE OVER-COUNT TRAP (load-bearing — flagged for review):** the AC gather here **must** use the full-tensor `cannon_gather` (take-my-block adjoint), **not** `cannon_gather_row`/`cannon_gather_col` (rules.jl:518/533, whose adjoints are reduce-scatter). Reduce-scatter is correct only when the downstream map is a *distributed-output contraction* where each rank contributed a genuine partial sum. The QR is a **replicated** computation, not a distributed contraction — using a reduce-scatter adjoint would over-count the AC gradient by ~P (cannon.jl:256). Symmetric trap on C: C is replicated and must **not** be gathered/scattered at all (no comm around it).
...
The determinism premise holds because `cannon_gather` is **pure placement** (allgatherv + deterministic per-block copy, cannon_2d.jl:193-201) — no floating-point reduction, so all ranks build the same `full` byte-for-byte. **Invariant to protect:** if any future variant introduced a *reduction* into the gather (summing overlapping blocks), bitwise replication could break and `safesign` could flip a column sign on one rank → divergent AL/AR → grid desync. Current gather is pure placement; the trajectory-match gate naturally catches a violation.

**[R1 — backward determinism, completeness]** The replicated-`dA` claim also covers the *backward* path: `dAL_full`/`dAR_full` feeding the `qrpos`/`lqpos` pullback are produced by the `cannon_scatter` allreduce adjoint (`allreduce_p2p!`, rules.jl:460), a rank-identical reduction, so the pullback inputs are replicated → `dAC_full` is replicated → exactly what the subsequent take-my-block requires.

**[R1 — bit-for-bit scope, F4]** Distinguish two determinism claims, only one of which is true:
- **(a) Across ranks: cannon AL/AR are bit-for-bit identical** — TRUE. Every rank gathers the same full AC and runs the same deterministic `qrpos`. This is the R-4 invariant; the gate asserts it directly (every rank's gathered full AL equal).
- **(b) Cannon AL == *serial* AL bit-for-bit** — **FALSE**, and the gate must NOT expect it. The AC fed to the seam comes from `ACenv_cannon`, whose power iteration sums block-locally + `cannon_norm` allreduces — a **different floating-point summation order** than serial `ACenv`'s single full sum → cannon AC ≠ serial AC at ~1e-13 → QR outputs differ at ~1e-12. Worse, `safesign` (misc.jl:125) can flip a near-zero R-diagonal column sign under that perturbation, a *per-column* gauge flip a global phase factor cannot absorb. So the cannon-vs-serial expectation is **rel ≤ 1e-10 up-to-gauge**, not bit-for-bit (see §6.1).
...
## 3. The entry seam — `ALCtoAC_cannon`

`ALCtoAC_map(AL::leg4, C) = @tensor result[a,b,c,e] := AL[a,b,c,d] * C[d,e]` (basic.jl:37). On blocks, AL's contracted last leg `d` is `r2`-split, while C is replicated full χ×χ. So a block-local contraction gives only a *partial* sum over the rank's `d`-slice → it needs a reduction over the `r2` partition. The output AC must come out as a block (first leg `a` by `r1` — inherited from AL; last leg `e` by `r2`) to seed `ACenv_cannon`.

**[R1 — why ALCtoAC must be distributed at all (parity-load-bearing)]** `ACenv_cannon` (cannon.jl:232) seeds `simple_eig` with this AC and runs a **finite** `power_iter_ad` in the AD loop (general.jl:945), so the post-step AC depends on the seed at the bit level. The §6.1 gate demands per-step trajectory match, so seeding `ACenv` with anything other than serial's `ALCtoAC(AL,C)` (e.g. a cheap block-local approximation or the previous AC) would diverge from serial and fail the gate. The cheap-seed shortcut is therefore rejected.
...
```julia
function ALCtoAC_cannon(AL_blk, C, grid)
...
- **Pros:** reuses the *exact same* primitives, kernel, and AD treatment as the QR seam (§2) → one uniform, already-tested seam pattern across both endpoints; lowest correctness risk; mirrors the `Cenv_cannon` precedent (gather → serial kernel). Zero new rrules.
- **Cons:** materializes full AL. **[R1 — F6 corrected]** At χ=256, D=8, complex128 the full AL is 256·8·8·256·16 B = **67 MB/cell** (the design's earlier "1.07 GB" was wrong by ~16×, having substituted χ for the middle D legs); at the χ=1024, D=10 production target it is **1.68 GB/cell**. Under `step_checkpoint=Recompute()` (which §5.3/R-6 already mandates for the deadlock argument) the forward full-tensor intermediates are freed and recomputed on backward, so per-step transients do **not** accumulate on the tape — Option A is **feasible at production**, not fatal. **[R1 — leg-d round trip]** A correctness-neutral inefficiency: the scattered AC's last (`r2`/`e`) leg is immediately re-gathered to full by `ACenv_cannon`'s first map (`ACmap_cannon_sliced` row-gathers it, cannon_2d.jl:622), so Option A does a wasted scatter→regather of that leg. Only the `a`(r1) split is genuinely used. This is a (small) extra argument for Option B, not a correctness issue.
...
Each rank contracts its `d`-slice of block AL against the matching rows of replicated C (sliced locally, no comm), producing a full-`e` partial; a **reduce-scatter over the `r2` partition** sums the `d`-partials and scatters `e` by `r2`. AD adjoint = col-allgather — both `cannon_gather_col`/`_cannon_col_reduce_scatter` already exist and are M4-tested (rules.jl:533/540).

- **Pros:** no full AL materialization → lower memory peak (matters at production χ, aligns with "cannon beats slice by avoiding full materialization").
...
```julia
function init_VUMPSRuntime_cannon(M, χ, grid, alg)
...
where `scatter_struct(S, grid) = StructArray([cannon_scatter(t, grid) for t in S.data], S.pattern)` (the validated `scatter_sa`, test_cannon_m4.jl:40), and `bcast_struct` maps `MPI.bcast` over `.data`.

**[R1 — F1, BLOCKING fix]** `initial_A → randSA` (initial.jl:23) uses the **default global RNG with no seeding and no bcast**, so every rank canonicalizes a *different* random `A`. Since `cannon_scatter` is pure local indexing (it slices *this rank's own* `A`, not a shared one), without the bcast every rank scatters a slice of a different tensor → the first `cannon_gather` in a step reassembles mismatched blocks → garbage. The **serial** init guards exactly this with `MPI.bcast(AL/AR/C, 0, COMM_WORLD)` — **but only `if alg.ifparallel`** (general.jl:780-784), and the cannon path runs `ifparallel=false`, so that branch is skipped. The cannon init must therefore bcast **unconditionally** over `grid.comm` (or seed the RNG identically; bcast is preferred — RNG seeding across CPU/CUDA is fragile). The M4 gate never hit this because it does `Random.seed!` then scatters one shared full tensor (test_cannon_m4.jl:31-38) — see §6 for the gate change that catches it.

### 4.2 Confirmed-feasible facts

- **`cannon_scatter` on each `.data` entry yields a block StructArray with no type change** (cannon_2d.jl:164 is pure indexing returning a smaller `Array`; StructArray is untyped over its data, base.jl:27-35). ✅ Already exercised by every M4 gate.
...
- **`update!`/`copy`/`Zygote.Buffer`/offload/`CuArray` are all `.data`-mapping → shape-agnostic** (environment.jl:179-216, structarray/base.jl:51-66). Block StructArrays flow through unchanged; the cannon env solvers already Buffer/copy block StructArrays (cannon.jl:73 etc.). ✅
...
Run serial `vumps_step` and `vumps_step_cannon` from the **same** initial runtime, for K steps, comparing every output tensor each step:

```
rt_serial = init_VUMPSRuntime(M, χ, alg_serial)            # full
rt_cannon = scatter the SAME rt_serial to blocks (C replicated)
for k in 1:K:
    rt_serial, err_s = vumps_step(rt_serial, M, alg)
    rt_cannon, err_c = vumps_step_cannon(rt_cannon, M, grid, alg)
    # gather rt_cannon's AL/AR/FL/FR to full; C already full
    @assert phase_ok(gather(rt_cannon.AL), rt_serial.AL)   # AL up to gauge
    @assert relerr(gather(rt_cannon.FL), rt_serial.FL) ≤ 1e-10
    @assert relerr(gather(rt_cannon.FR), rt_serial.FR) ≤ 1e-10        # FR
    @assert phase_ok(gather(rt_cannon.AR), rt_serial.AR)              # AR — SEPARATE lqpos path
    @assert relerr(rt_cannon.C, rt_serial.C) ≤ 1e-10                  # C (already full, replicated)
    @assert abs(err_c - err_s) ≤ 1e-10
    # cross-rank invariant (R-4): every rank's gathered AL/AR + replicated C bitwise-equal
    @assert MPI.Allreduce(maxabsdiff(gather(rt_cannon.AL), rank0_AL), max, grid.comm) == 0
    @assert MPI.Allreduce(maxabsdiff(rt_cannon.C, rank0_C), max, grid.comm) == 0
```

**[R1 — F4/F5 gauge expectation, corrected]** The cannon-vs-serial expectation is **rel ≤ 1e-10 up-to-gauge**, **not** bit-for-bit. Reason: the AC fed to the QR seam comes from `ACenv_cannon`'s power iteration, whose block-local-sum + `cannon_norm`-allreduce gives a *different FP summation order* than serial `ACenv`'s single full sum (~1e-13), so cannon AL/AR differ from serial at ~1e-12. Compare AL **and AR** (separate `qrpos`/`lqpos` paths) with `phase_ok`/`phase_relerr` (M4 helpers); drop the circular "AC-implied" (there is no AC output field on `VUMPSRuntime`). Bit-for-bit holds **only across ranks** (every rank gathers the same full AC) — asserted separately above. Guard against per-column `safesign` sign-flips by keeping the test fixture's R-diagonals bounded away from zero (or use a per-column-sign-robust phase compare).

Seed the comparison from the SAME `rt` (scatter serial's init) rather than two independent inits, so step k compares like-for-like without accumulated trajectory drift masking a per-step bug. **[R1 — F1]** *Additionally* run one gate variant that constructs `rt_cannon` via `init_VUMPSRuntime_cannon` (independent per-rank init + bcast), **not** scatter-of-serial — this is the only configuration that catches the un-bcast'd RNG bug (F1); scatter-of-serial seeding masks it.
...
- **Loss must be LINEAR with a scattered fixed weight** (the M4 pattern, test_cannon_m4.jl:190), not a quadratic on a block. A block `dot(AL_blk,AL_blk)` is a per-rank *partial* of the global quadratic, so its block cotangent is the gradient of the *local* loss, ≠ `blkof`(global-loss gradient). Cannon side: `L = real(sum_c sum(conj(Wb[c]) .* AL_out.data[c])) + (AR term)` with `Wb[c]=cannon_scatter(W[c],grid)`; serial side: same with full `W`. (A quadratic is acceptable only if built from `cannon_dot`, the *global* inner product, cannon_2d.jl:212.)
...
- **Negative control:** include a deliberately-wrong variant that gathers AC via `cannon_gather_row` and assert the gate *fails* (over-count factor = **N1 or N2**, i.e. 2 on a 2×2 grid — the reduce-scatter sums over one axis only, so it is N1/N2, not the full P). Proves the gate actually catches R-1.
- Run under `step_checkpoint ∈ {Plain, Recompute}` (Recompute is the deadlock-relevant mode; Offload is a CPU pass-through, skip it).

### 6.3 Execution plan

- **CPU 4-rank (2×2 grid)** first — fast iteration, catches the over-count (N1/N2=2× per-block), the init-bcast bug (F1, via the `init_VUMPSRuntime_cannon` variant), AD composition, the Recompute deadlock. New `test/test_cannon_m5.jl` mirroring the M4 gate structure.
...
- **GPU 16-rank (4×4 grid)** on Sofia — χ=256 D=8, the production regime, after CPU is green. New `examples/MPI_parallel/test_cannon_m5_sofia.jl` + submit script, mirroring the M4 GPU driver (reuse the GPU-safe `phase_relerr` with `dot(b,a)/dot(b,b)`, NOT scalar-indexing).
...
|---|---|---|
| R-1 | **Over-count** if the AC gather uses row/col reduce-scatter instead of take-my-block | §2.3 — use full `cannon_gather`; factor is **N1/N2** (per-axis), not P; §6.2 gradient gate + negative control catches it |
...
| R-8 | Cannon-vs-serial expects bit-for-bit but bug hides in gauge | §6.1 — **R1-corrected: expect rel ≤ 1e-10 up-to-gauge** (upstream power-iter FP order differs); bit-for-bit only *across ranks* |
| R-9 | Wrong step variant copied (`vumps_step_power` vs `vumps_step`) | §1 — mirror `vumps_step` (old AL/AR into envs, single solve) |
| **R-10** | **[R1-F1] Init RNG not bcast → per-rank-divergent full A → mismatched blocks** | §4.1 — **bcast over `grid.comm` UNCONDITIONALLY** in cannon init; §6.1 gate runs the independent-init variant |
| R-11 | [R1] `vumps_itr` warm-up loop (general.jl:931) not routed to cannon step → block tensors hit serial step | §5.1 — route **both** call sites (931 + 977) on `alg.grid !== nothing` |
...
- [ ] `ACCtoALAR_cannon(AC_blk, C, grid)` — gather AC StructArray, **verbatim serial `ACCtoALAR`**, scatter AL/AR (cannon.jl, new).
- [ ] `ALCtoAC_cannon(AL_blk, C, grid)` — Option A v1: gather AL StructArray, **verbatim serial `ALCtoAC`**, scatter AC.
- [ ] `vumps_step_cannon(rt, M, grid, alg)` — 6-call orchestration (mirror `vumps_step`, NOT `vumps_step_power`).
- [ ] `init_VUMPSRuntime_cannon(M, χ, grid, alg)` — serial build + **unconditional bcast over `grid.comm`** + scatter (R-10).
- [ ] Dispatch: `alg.grid::Union{Nothing,CannonGrid}` field + `vumps_itr` routing at **both** general.jl:931 (warm-up) and :977 (AD) — Option 2.
- [ ] `test/test_cannon_m5.jl` — CPU 4-rank trajectory + gradient parity; linear-scattered-weight loss on AL+AR, per-block compare; Plain + Recompute; all-distinct **and** `[1 2;2 1]` patterns; init-bcast variant; negative control.
- [ ] `examples/MPI_parallel/test_cannon_m5_sofia.jl` + submit — 16-GPU validation.
...
|---|---|---|---|
| **F1** | 🔴 blocking | Init runs `initial_A` with un-seeded per-rank RNG; serial bcast is gated on `ifparallel` (false for cannon) → divergent full A → mismatched blocks. | **Fixed §4.1** — unconditional `bcast over grid.comm`. New R-10. Gate runs the independent-init variant (§6.1). |
...
Implemented exactly per §8 in `src/boundary_algorithm/vumps/cannon.jl` (M5 section after `Cenv_cannon`): `gather_struct`/`scatter_struct`/`bcast_struct`, `ALCtoAC_cannon`, `ACCtoALAR_cannon`, `vumps_step_cannon`, `init_VUMPSRuntime_cannon`; `grid` field on `VUMPS` (interface.jl); a 1-line routing guard at the top of `vumps_step` (general.jl) covering both `vumps_itr` call sites. Gate: `test/test_cannon_m5.jl` (4-rank CPU) + `examples/MPI_parallel/test_cannon_m5_sofia.jl` (16-GPU) + submit/smoke.

### The AL/AR conditioning discovery (supersedes the §6.1/F4 "compare AL/AR up-to-gauge" plan)

During gating, comparing the full step's **final AL/AR** directly between serial and cannon failed by **O(1)** for a random (non-canonical) test rt — while FL/FR/C/err matched to **machine precision** (~1e-16) and the seam was **bit-exact** (M5-0). Root-caused through a chain of diagnostics:

1. `ACenv_cannon`'s intermediate AC (the seam input) matches serial to **4e-16** per-cell, all patterns/power_iter (`diag_ac`).
2. In the full step, cannon AL = serial `ACCtoALAR` on cannon's gathered AC + cannon C **exactly** (`ALc_vs_ALsc = 0`); the divergence is entirely `ACCtoALAR(sameAC, Cc)` vs `ACCtoALAR(sameAC, Cs)` (`diag_step`).
...
- **M5-3** — `init_VUMPSRuntime_cannon` cross-rank bitwise consistency (F1/R-10 catcher). (5 tests.)
- **M5-4** — `vumps_step` routes to `vumps_step_cannon` when `alg.grid` set (R-11). (5 tests.)

**Result: all 59 CPU gates GREEN.** GPU driver (`test_cannon_m5_sofia.jl`) mirrors this corrected validation (no direct AL/AR; intermediate AC + seam grads with well-conditioned C; `SEAM_RTOL=1e-10` for cuSOLVER) and PASSES the 4-rank CPU smoke. Ready for 16-GPU Sofia.
