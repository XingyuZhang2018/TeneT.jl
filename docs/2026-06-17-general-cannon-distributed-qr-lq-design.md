# General Cannon distributed QR/LQ seams

Date: 2026-06-17. Scope: replace the full-gather QR/contraction seams in
`VUMPS{General}` Cannon steps. This is the follow-up to the Plaquette TSQR
path and makes the General runtime obey the Cannon rule: production code must
not materialize full `AL`, `AR`, `AC`, `FL`, or `FR` on every rank.

## Goal

Make `vumps_step_cannon(rt::VUMPSRuntime, M, grid, alg::VUMPS{General})`
block-distributed end to end for the persistent boundary tensors:

- `AL`, `AR`, `AC`, `FL`, and `FR` stay in Cannon blocks
...
The current General Cannon step still contains two full-gather seams:
...
`ALCtoAC_cannon` gathers full `AL`, calls serial `ALCtoAC`, and scatters full
`AC`. `ACCtoALAR_cannon` gathers full `AC`, calls serial `ACCtoALAR`, and
scatters full `AL`/`AR`. This breaks the Cannon memory model. The Plaquette
branch added `ACCtoAL_tsqr_cannon`, but General still lacks the matching
distributed LQ path for `ACCtoAR`.

There is also an `ALCtoAC_cannon_dist` helper used by observable code. It is a
temporary side path rather than the canonical General seam. This design replaces
it with one production `ALCtoAC_cannon` implementation and renames the old
...
- Do not change the Cannon FL/FR/AC/C map algorithms except where their
...
- Do not support rectangular grids for the new QR/LQ seams in the first phase.
  The existing General Cannon path already assumes square-grid behavior in the
  cross-axis maps.

## Design overview

Add three distributed seam primitives and route General `vumps_step_cannon`
through them:

```julia
AC = ALCtoAC_cannon(AL, C, grid)                  # distributed contraction
AL, AR, errL, errR = ACCtoALAR_dist_cannon(AC, C, grid)
```

The production meanings become:

- `ALCtoAC_cannon`: distributed contraction, no full `AL` or `AC`.
- `ACCtoAL_tsqr_cannon`: distributed left QR, already present for Plaquette and
  promoted to the shared General/Plaquette implementation.
- `ACCtoAR_tslq_cannon`: new distributed right LQ.
- `ACCtoALAR_dist_cannon`: calls both distributed exits and preserves the
  serial residual accumulation rules.

The old gather wrappers are renamed:

- `ALCtoAC_cannon_gather_ref`
- `ACCtoAL_cannon_gather_ref`
- `ACCtoALAR_cannon_gather_ref`

Tests use reference helpers for parity. Production `vumps_step_cannon` does not.
...
2. Sum over `row_comm` and keep this rank's `d` block:
   ...
   ```julia
   AC_blk = _cannon_row_reduce_scatter_last(partial, grid, d_ranges)
   ```

This removes the row allgather in `ALCtoAC_cannon_dist` and gives the operation
...
- The allreduce for `dC` is required because `C` is a replicated input; this is
  the same replicated-operand rule used elsewhere in Cannon AD.

## Distributed left QR: `ACCtoAL_tsqr_cannon`
...
1. Row-gather the last chi leg:
   ...
   ```julia
   AC_row = cannon_gather_row(AC_blk, grid, last_ranges)
   ```
...
4. Gather the small `Rloc` factors over `col_comm`, stack them, and QR again:
   ...
   ```julia
   Rstack = cannon_gather_col(Rloc, grid, r_ranges)
...
- `cannon_gather_row` and `cannon_gather_col` provide the matching
...
## Distributed right LQ: `ACCtoAR_tslq_cannon`
...
- The gather adjoints are axis-swapped from left TSQR:
  `cannon_gather_col` backpropagates through `_cannon_col_reduce_scatter`.
...
`ACCtoALAR_dist_cannon` must preserve this asymmetry. The implementation should
not blindly map both exits over `eachindex(AC.data)` unless it also matches the
serial residual and cell-selection semantics.

Required structure:

- `ACCtoAL_tsqr_cannon` returns the same `AL` data and `errL` as serial
  `ACCtoAL`, including repeated-pattern multiplicities.
- `ACCtoAR_tslq_cannon` returns the same `AR` data and `errR` as serial
  `ACCtoAR`, using the first occurrence and the serial `jr` rule.
- `ACCtoALAR_dist_cannon` simply combines those two outputs.
...
function vumps_step_cannon(rt::VUMPSRuntime, M::StructArray, grid::CannonGrid, alg::VUMPS{General})
    ...
    _, C  = Cenv_cannon(C, FL, FR, grid; alg)
...
Plaquette may continue to use `ACCtoAL_tsqr_cannon`. After the General seam is
stable, the `distributed_qr` flag should be reinterpreted or removed so the
production Cannon path has one meaning instead of "gather by default, TSQR only
when opted in." During migration, keep the flag only as a compatibility switch.

Observable code should call `ALCtoAC_cannon`, not `ALCtoAC_cannon_dist`.

## Test plan

Add a 4-rank CPU MPI test file, e.g. `test/test_cannon_general_dist_qr_lq.jl`.

Forward seam tests:

- `ALCtoAC_cannon` vs `scatter(ALCtoAC(gather(AL), C))`.
- `ACCtoAL_tsqr_cannon` vs serial `ACCtoAL`.
- `ACCtoAR_tslq_cannon` vs serial `ACCtoAR`.
- `ACCtoALAR_dist_cannon` vs serial `ACCtoALAR`.
...
- One General `vumps_step_cannon` parity test against serial at small chi.
...
1. Rename existing gather seams to `*_gather_ref` and update tests to call those
   names explicitly.
2. Implement `ALCtoAC_cannon` as the row reduce-scatter contraction.
3. Generalize the TSQR helper so left QR can use `col_comm` and right LQ can use
   `row_comm`.
4. Keep `ACCtoAL_tsqr_cannon` as the shared left QR seam and fix residual
   accumulation if repeated-pattern tests expose a mismatch.
5. Implement `ACCtoAR_tslq_cannon`, `lqpos_colrep`, and `ACCtoALAR_dist_cannon`.
6. Route General `vumps_step_cannon` to the distributed seams.
7. Replace observable calls to `ALCtoAC_cannon_dist` with `ALCtoAC_cannon`.
8. Remove `ALCtoAC_cannon_dist` once all callers are migrated.
9. Leave General init unchanged in this phase, but document it as the next
   remaining full-chi Cannon gap.
...
- General Cannon `vumps_step_cannon` contains no production call that gathers a
  full `AL`, `AR`, `AC`, `FL`, or `FR`.
- `ALCtoAC_cannon_dist` has no production callers.
