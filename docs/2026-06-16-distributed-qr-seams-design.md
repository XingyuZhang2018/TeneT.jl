...
- The **AD (gradient) pass OOMs** (`_cannon_fold2`, baseline near 143 GB). forloop_iter 16→64 shrank
  ...
  - `ALCtoAC_cannon` (cannon.jl:349): **gather full χ768 AL** (4 cells × 2.42 GB ≈ 9.6 GB) → serial
    `ALCtoAC` → scatter. Every vumps step.
  - `ACCtoAL_cannon` / `ACCtoALAR_cannon` (cannon.jl:556/370): **gather full χ768 AC** (≈9.6 GB) →
...
- `AC_blk = reduce_scatter_row_last(partial)` — sums the partials over r2 + scatters the output `d`
  to r2-blocks. NO gather. Reuses `_cannon_row_reduce_scatter_last` (cannon_2d.jl:402, NCCL path +
...
- Phase 1: `ALCtoAC_cannon(AL_blk, C)` (reduce-scatter) vs `scatter(ALCtoAC(gather(AL_blk), C))` —
...
- Phase 2: `ACCtoAL_cannon` (TSQR) vs serial `ACCtoAL(gather(AC_blk), C)` — AL up-to-gauge (QR gauge:
...
- GPU: re-run the χ768 smoke; expect the AD to fit (no `_cannon_fold2` OOM) and `SMOKE forward
...
5. C replicated throughout (both seams) — its cotangent handling (allreduce vs not) must match the
   cannon_dot/cannon_norm asymmetry rule.
...
vumps_step, …)`, plaquette.jl:268), already gated, UNBLOCKED on the cannon path (`vumps_step_cannon`
only asserts `inner_checkpoint isa Plain`; step/subop are free). Collapses the 4-step tape to ~1.
CPU-validated (test/local_cannon_realphys.jl, step_checkpoint=Recompute): Phase-1 cannon==serial==
-0.470392432614 (Δ2.2e-16) AND Phase-2 descent BIT-IDENTICAL to Plain (recompute is exact; the
cannon recompute-in-backward re-runs the MPI collectives rank-uniformly, fine).
...
`_cannon_row_reduce_scatter_last`) was confirmed correct by R1 — keep for (2).
