# M3.5: make FRmap / ACdmap slice2d RING-class by reordering the local chain
...
of `FRmap_slice2d_dist`/`ACdmap_slice2d_dist` is **not fundamental** — it is an
...
Predecessor: `docs/2026-06-13-acmap-slice2d-dataflow-design.md` (the gather-class
...
All four maps share the uniform Slice2D block convention (first χ leg → N1/r1,
...
```julia
const ACDMAP_LEG5_SLICE2D_CHAIN = tensor_chain(
    ((:a,:e,:f,:i), (:i,:j,:k,:l), (:e,:j,:g,:b,:p), (:f,:k,:h,:c,:p), (:d,:g,:h,:l)), (:a,:b,:c,:d))
const FRMAP_LEG5_SLICE2D_CHAIN  = tensor_chain(
...
(`acmap-slice2d-dataflow-design.md` §5.1) to the reorder:

| step | current (gather-class) | reorder (ring-class) |
|------|------------------------|----------------------|
| χ recovery (F0) | `MPI.Allreduce` size over col_comm | **same** |
| gather i on ACd | `_slice2d_col_allgather` (730) | **same** |
| gather i on FL | `_slice2d_row_allgather` (750) | **same** |
| gather d on FR | `_slice2d_col_allgather` (730) | **same** |
| local einsum | 2-level i/d chunk, `ACDMAP_LEG5_CHAIN` | **single-l chunk, `ACDMAP_LEG5_SLICE2D_CHAIN`** ← only this changes |
| output | `_slice2d_row_reduce_scatter_last` (760): Σ_l over row + scatter d→r2 | **same** |
...
`chain_backward(ACDMAP_LEG5_SLICE2D_CHAIN, (FL_g, ACd_g, M1, M2, FR_g), dpartial)`
...
1. **B4 unpack order follows the NEW ops order.** `chain_backward` returns grads
   ...
   `(dFL_c, dACd_c, dM1_c, dM2_c, dFR_c) = chain_backward(ACDMAP_LEG5_SLICE2D_CHAIN,
...
1. **Chains** (`chain_maps.jl`): add `ACDMAP_LEG5_SLICE2D_CHAIN`,
   `FRMAP_LEG5_SLICE2D_CHAIN` (+ single-M `conj_variant` twins). **Guard against a
...
2. **Forward** (`slice2d.jl` `_acdmap_slice2d_forward_sliced`,
   `_frmap_slice2d_forward_sliced`): keep F1–F3 gathers + F5 reduce-scatter
   verbatim; replace the F4 2-level i/d loop with a single-l-chunk loop over the
   `_SLICE2D_CHAIN`. Drop the `n_i,n_d`/`n_chunk = N·⌈√floop⌉` machinery; chunk
   `l` like `_slice2d_forward_sliced` (FLmap) does.
3. **Backward** (`rules.jl` FRmap/ACdmap rrules): swap `chain_backward` to the
   `_SLICE2D_CHAIN`; chunk loop 2-level→single-l; comm adjoints unchanged.
...
5. **Validate**: `test/test_slice2d_m3.jl` 4-rank parity (1e-12/1e-10) must stay
...
- **Single-M conj slot**: the M2 operand sits at a different chain position in
  ...
  `FLmap_slice2d_dist` precedent). Covered by the single-M parity test.
