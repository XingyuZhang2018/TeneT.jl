# Slice2D-Style 2D Distributed FLmap — Design
...
to a Slice2D-style two-stage ring. Partition convention and the rrule adjoint table
...
Slice2D-style distribution stores the environment in N1×N2 blocks across ranks.
...
| Approach | Verdict |
|----------|---------|
| **A. Two-stage Slice2D ring (chosen)** | Point-to-point shifts, zero new deps on existing MPI Isend/Irecv infra, NCCL p2p addable later |
...
`Slice2DGrid`: N1×N2 ranks, P = N1·N2. Rank → coordinates `(r1, r2)` with
...
### v3 (2026-06-12): ALu/ALd block-distributed too (`FLmap_slice2d_dist`)
...
- `ALd[:, :, :, l_r2]` (full i, local l) = 1/N2 of ALd — assembled by a
  **column allgather along the first leg** (the existing `_slice2d_col_allgather`
  pattern);
...
`_slice2d_col_reduce_scatter` semantics). Backward communication drops
...
2. **Per l-chunk, fully local and Zygote-free** (2026-06-12: Zygote removed
   entirely from `slice2d_back` — its tapes and `@tensor`-internal temporaries
...
```julia
# src/contraction/slice2d.jl  (new file, included after forloop_parallel_MPI.jl)
slice2d_grid(N1, N2; comm=MPI.COMM_WORLD) -> Slice2DGrid   # cached; row/col sub-comms
FLmap_slice2d(FL_blk, ALu, ALd, M, grid; inner_etype=nothing) -> result_blk
slice2d_scatter(FL_full, grid) -> FL_blk   # boundary shim: slice local block (adjoint: embed + allreduce)
slice2d_gather(blk, grid) -> full          # boundary shim: 2D allgather (adjoint: slice)
```
...
- `slice2d_scatter/gather` get rrules so map-level tests run end-to-end under
...
### Level 1 — local CPU correctness (`test/test_slice2d.jl`, `mpiexec -n 4`)

Pattern follows `test/test_mpi.jl`. Small random tensors (χ=16, D=4, ComplexF64, leg5).

- Forward parity: `slice2d_gather(FLmap_slice2d(slice2d_scatter(FL), …))` vs serial
  `FLmap`, rel. err ≤ 1e-12
- Gradient parity: full Zygote chain (scatter → slice2d → gather → scalar loss) vs
...
- Submit script `examples/MPI_parallel/Sofia/submit_test_slice2d.sh` using the
...
- Production-like sizes D=10, χ=400, leg5 tuple, 2×2 grid:
  ...
  - **peak memory per rank** (CUDA pool stats): Slice2D vs slice, validating the
...
`xz/TeneT_slice2d_sadsaha` (production checkout untouched). χ=400, D=10,
...
| **Slice2D 2×2** | **0.130 s** | **0.566 s** |

Slice2D is 2.1× faster forward and 1.6× faster fwd+bwd at 2×2 — better than
...
| slice2d fwd+bwd retained | 96.7 GB |

Honest reading: at map level this driver does NOT demonstrate a memory win —
the slice2d section retains more because the test scaffolding materializes
full tensors on every rank (`slice2d_scatter`/`slice2d_gather` per timed call)
...
- Replace the per-step `Zygote.pullback(_slice2d_stage1, …)` with the
