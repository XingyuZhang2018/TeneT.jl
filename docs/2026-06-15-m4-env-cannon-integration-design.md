# M4: env-level Cannon integration + gather hoisting (distributed leftenv/rightenv/ACenv/Cenv)

Status: **Batch A COMPLETE — implemented, 4-rank CPU parity GREEN, dual-reviewed (both
approve-with-nits, all nits addressed).** leftenv_cannon gates: Gate 1 eigenpair parity
...
equivalence), Gate 3a rrules **6/6** (complex; cannon_norm tested with distinct per-rank
...
handling); existing run_test_cannon.jl no-regression. Design adversarially reviewed (8-lens,
R1) + revised; the **cannon_norm cotangent-allreduce** was a Batch-A discovery (§5.3). Dual
review applied: GPU-densify fix in the gather-wrapper rrules (collect→device-aware similar),
`_free!(dpartial)`, CRLF→LF on misc.jl/TeneT.jl (clean diff). **Batch B (rightenv_cannon) + C
(ACenv_cannon) ALSO COMPLETE + 4-rank CPU GREEN** (Gate B1 36/36, B3 3/3, C1 pass incl. Ni≠Nj,
C3 9/9 incl. the non-leading-cell `cannon_norm` normalize under AD; dual review running). The
...
`cannon_norm` on non-leading cells). **Batch D (Cenv_cannon, replicated-output, take-my-block via
the existing `cannon_gather` + serial Cmap + plain dot/norm) DONE + green.** **Sofia 16-GPU 4×4
...
cannon rrules). **Remaining: F (NCCL χ·D/√P gating). M5 = vumps_step + QR gather seam.**
Branch: `claude/ecstatic-golick-e3f6f0` (M1+M2+M3+M3.5+NCCL landed; `CHAIN_ENGINE`
default ON; all 4 cannon maps single-l-chunk ring-class with identical comm).
Milestone row: `docs/2026-06-12-deterministic-contraction-engine-design.md` L125 —
**M4 = leftenv/rightenv/ACenv/Cenv cannon; gather hoisting; buffer reuse; gate =
...
Predecessors:
- `docs/2026-06-13-acmap-cannon-dataflow-design.md` (M3 cross-axis dataflow),
- `docs/2026-06-15-m35-cannon-ring-reorder-design.md` (M3.5 all-four-ring-class),
...
1. **Env layer is greenfield for cannon.** No `CannonGrid`/`cannon_*`/`grid::` anywhere
   ...
   incompatible** with the 2D-block Cannon scheme (block-in/block-out, reduce-scatter).
2. **The cannon maps are an AD-complete library with no production consumer.** Every
   `*_cannon_dist`/`Cmap_cannon` reference is in `cannon_2d.jl`, its rrule in `rules.jl`,
...
3. **`simple_eig` distribution hooks exist but are unwired.** `simple_eig(f, v; …,
   ...
   `cannon_dot`/`cannon_norm` (`cannon_2d.jl:212-218`) are built for them; **no caller
...
4. **The gather-hoist hook is already clean.** `_cannon_forward_sliced` (and the FR/AC/ACd
   `_*_cannon_forward_sliced`) take **already-gathered** slices; the `*_cannon_dist`
   wrappers are the only place the per-call gathers run. `FLmap_cannon_dist`'s own
   docstring (`cannon_2d.jl:524-526`) flags hoisting as "the natural optimization
   (leftenv round)."
...
6. **AD differentiates the env via `pullback(vumps_itr)`** (`rules.jl:1074`, the
   ...
   recompute-style under `checkpoint(eig_checkpoint)`. So cannon map rrules compose
...
8. **No env-level / hoisting / `simple_eig`-with-cannon-hooks test exists** — only a
   hand-rolled FLmap power loop (`test/test_cannon.jl:248-282`). A hoisting bug (stale
...
9. **Persistent comm buffers exist for the base collectives** (`_ensure_buf!` +
   ...
   **cannon collectives allocate fresh per call** (`cannon_2d.jl` ring/gather/RS helpers).
...
10. **Runtime types need no change.** `StructArray = {data::Vector{dense}, pattern}`; a
    ...
    shapes. Obstacles are semantic: `norm`/`dot` are **local** on blocks (→ cannon
    ...
    cannon block convention exactly.
...
12. **FR/ACd distributed backward MUST use the `_CANNON` reordered chains** via
...
- **R1-B1 `cannon_dot` rrule formula was wrong for complex.** Julia `dot(x,y)=Σ conj(x)·y`
  is antilinear in x. Correct (ChainRules dense.jl convention; `ds` is the replicated
  allreduce-output cotangent → no extra comm):
  `rrule(cannon_dot)(ds) = (NoTangent, y_blk·conj(ds), x_blk·ds, NoTangent)`.
- **R1-B2 `cannon_norm` rrule formula was wrong** (dropped the real-projection). Correct
  form: `rrule(cannon_norm)(dn) = (NoTangent, x_blk·(real(c̄)/n), NoTangent)` with **global**
  ...
  `cannon_dot` stays local (consumed as a scalar). VERIFIED GREEN.
- **R1-B3 `orth_for_ad` uses a LOCAL `dot` (rules.jl:99: `_dv - dot(v,_dv)·v`) — wrong on
  ...
  required hook, not just dot/norm.** Fix: add a grid-aware `orth_for_ad_cannon(v, grid)`
  whose rrule uses `_dv - cannon_dot(v,_dv,grid)·v`, and give `simple_eig` an `orth_fn`
  kwarg (defaulting to `orth_for_ad`) threaded by `leftenv_cannon`/`rightenv_cannon`/
  `ACenv_cannon`. **Cenv is exempt** (C replicated → local dot == global). Verified
  first-hand at rules.jl:95-103.
- **R1-B4 Cannon maps are leg5-ONLY; "General-mode core" wrongly implied leg4/leg8-M
  parity.** `_cannon_stage1`/the `*_CANNON`/`ACMAP_LEG5` chains are hard-wired to leg5 FL
  + leg4 AL + leg5 M; only `Cmap_cannon` handles leg3/leg4. The serial General env also
  serves **leg4 single-layer M (2D-classical Ising, leg3 FL)** and **leg8** — unachievable
  with today's cannon maps. **Fix:** §1 restricts M4 to **leg5 single-layer-pair M (the
  iPEPS General path)**; leg4/leg8-M → §11 deferred (cannon leg4/leg8 maps do not exist).
  M is passed as a single leg5 tensor; `*_cannon_sliced` materializes `conj(M)` (no `_1M`
  cannon chain — a perf, not correctness, gap, flagged).

### Majors

- **R1-M1 The sliced-core surgery was mischaracterized for FR/AC/ACd.** Their
  `_*_cannon_forward_sliced` cores take **all** operands *already gathered, including the
  iterate* (`_frmap_cannon_forward_sliced(ARd_g, FR_g, ARu_g, …)`, cannon_2d.jl:612). So
  `FRmap_cannon_sliced` is **not** "reuse the core unchanged": it is a thin wrapper that
  gathers **only the iterate** per-call (`FR_g = _cannon_col_allgather(FR_blk,…)`) then
  ...
  carried block, never gathered → `FLmap_cannon_sliced` calls `_cannon_forward_sliced`
...
- **R1-M2 The `eig_checkpoint=Recompute()` boundary was dropped.** The serial env wraps the
  ...
  **the hoisted gathers must sit OUTSIDE the checkpoint**, and a new
  `_simple_eig_*_cannon(iterate_blk, ALu_row, ALd_col, M, grid; inner_product, norm_fn,
...
- **R1-M3 `cannon_gather_full` duplicates the existing `cannon_gather`** (rules.jl:450
  already has the take-my-block rrule). **Reuse `cannon_gather`** for Cenv's full gather
  (or make `cannon_gather_full` a documented alias); its rrule must slice **both** grid
  axes (`dfull[a_rs[r1+1], …, e_rs[r2+1]]`). The naming separation from
  `cannon_gather_row/col` is by-convention only — add a **negative test** (feed Cenv's FL
...
- **R1-M5/M6 NCCL gating.** (a) The gate predicate must take the **GLOBAL χ** (the
  `MPI.Allreduce(size(blk,1),+,col_comm)` value, cannon_2d.jl:540) + grid P + replicated D
  ...
  **χ·D/√P** (constant at both measured crossovers: 512/4 = 1024/8 = 128, ×D); the
  ...
  patched §7.
- **R1-M7 Gate 1 (eigenpair parity) is wrong for the replicated C** — do NOT `cannon_gather`
...
- **R1-M9 Double-buffered iterate is UNSAFE under AD.** The forward aliases the iterate into
  the rrule-captured `blocks` (cannon_2d.jl:440-443; rrule reads `blocks[t+1]` in the
...
- **R1-M11 The zero-alloc gate using gross `@allocated` is meaningless** — it counts every
  ...
  warm-up, and (b) a **direct counter** of cannon-collective `similar`/pool reallocations
...
- **R1-M12 Mixed precision must fail LOUD, not silent.** `leftenv_cannon` gets the same
  `alg`; if `alg.inner_etype`/`whole_vumps_etype`/`simple_eig_polish_steps` are set, the
  cannon path would silently run full precision while serial downcasts. Add
  `@assert alg.inner_etype === nothing && alg.whole_vumps_etype === nothing` (+ polish) at
  each `*_cannon` entry. Also `@assert alg.ifsimple_eig` (the eigsolve/GMRES branch is an
  implicit-adjoint that bypasses the cannon_dot/norm/orth rrules → out of scope) and
...
- **R1-M13 ACenv axis transposition must be written out.** ACenv loops the unit-cell
  **column j** (outer), chains over the **row i**, uses `selectpos(…, Ni)`, non-leading
  ...
  a **different `i`** from the unit-cell row index. Explicit `ACenv_cannon` and
  `Cenv_cannon` pseudocode added (§2.3'). Gate 1 adds an Ni≠Nj unit cell (e.g. 1×2) so a
...
- **R1-m3** The §3 ACdmap row is **moot for M4** — no env function power-iterates ACdmap
  (its only consumer is the deferred obs env, applied once). No `ACdmap_cannon_sliced`/
  `ACdenv_cannon` in M4. Carried for the M5 obs path.
- **R1-m4** The FD micro-test of cannon_dot/cannon_norm/orth_for_ad_cannon MUST use
  **complex** inputs + a complex upstream cotangent (real inputs hide the conj/real bugs).
- **R1-m5** Hard precondition: **`ifsimple_eig=true`** on the cannon env (folded into R1-M12).
- **R1-m6** `cannon_gather`'s take-my-block adjoint slices **both** grid axes (§ R1-M3).
...
```
leftenv_cannon (ALu_blk, ALd_blk, M, FL_blk, grid; alg)  →  (λL, FL_blk')
rightenv_cannon(ARu_blk, ARd_blk, M, FR_blk, grid; alg)  →  (λR, FR_blk')
ACenv_cannon   (AC_blk, FL_blk, M, FR_blk, grid; alg)    →  (λAC, AC_blk')
Cenv_cannon    (C_full, FL_blk, FR_blk, grid; alg)       →  (λC, C_full')
```
...
cannon functions live alongside, dispatched explicitly by the (future M5) cannon
...
(sliced-map rrules + gather-wrapper rrules + cannon_dot/cannon_norm rrules) so the
...
`vumps_step`/`vumps_itr`/`leading_boundary` cannon assembly, `init_VUMPSRuntime`
block scatter, mixed-precision (`inner_etype`/`whole_vumps_etype`) on the cannon env
...
A **single** `leftenv_cannon` must serve both. The design achieves this with one uniform
...
**(a) Gather wrappers** — `cannon_gather_row(blk, grid, rs)` and
`cannon_gather_col(blk, grid, rs)`, thin differentiable wrappers over the existing
`_cannon_row_allgather` / `_cannon_col_allgather`. Their rrule adjoints are the existing
reduce-scatters:
```
rrule(cannon_gather_row): forward = _cannon_row_allgather;  back(d) = _cannon_row_reduce_scatter_last(d, grid, rs)
rrule(cannon_gather_col): forward = _cannon_col_allgather;  back(d) = _cannon_col_reduce_scatter(d, grid, rs)
```
...
**(b) Sliced map entry points** — `FLmap_cannon_sliced`, `FRmap_cannon_sliced`,
`ACmap_cannon_sliced` (NO `ACdmap_cannon_sliced` — R1-m3: no M4 env iterates ACdmap). Each
...
- **FLmap (clean):** the iterate FL is the ring carried block, never gathered.
  `FLmap_cannon_sliced(FL_blk, ALu_row, ALd_col, M, grid; forloop_iter)` calls
  `_cannon_forward_sliced` directly.
- **FRmap/ACmap (iterate-gather):** the existing `_*_cannon_forward_sliced` cores take ALL
  operands *already gathered including the iterate* (`_frmap_cannon_forward_sliced(ARd_g,
  FR_g, ARu_g, …)`, cannon_2d.jl:612). So the sliced wrapper gathers **only the iterate**
  per-call (`FR_g = _cannon_col_allgather(FR_blk,…)`; `AC_g = _cannon_row_allgather(AC_blk,…)`)
...
_cannon_col_reduce_scatter(dFR_g)` stays per-call). It returns **slice** gradients for the
hoisted fixed operands and the block gradient for the iterate:
```
FLmap_cannon_sliced rrule returns:  (NoTangent, dFL_blk, dALu_row, dALd_col, dM, NoTangent)
                                      (^iterate)   (^slice)   (^slice)
```
The type-A output-adjoint allgather of `dresult` (`_cannon_col_allgather(d_c,…)` /
`_cannon_row_allgather(d_c,…)`) **stays inside** this rrule — it is per-iterate (a
...
densify guard — all unchanged from the `*_dist` rrule. FR/ACd use the `_CANNON`
reordered chains via `chain_backward` (research finding #12); FL uses its hand kernels.

**(c) Distributed inner-product / norm** — `cannon_dot`/`cannon_norm` gain rrules so the
differentiated normalization (`v /= norm_fn(v)`) and eigenvalue
(`λ = inner_product(v, v1)`) are correct on blocks (§5.3).

### 2.3 What the env function does (`leftenv_cannon`; the checkpoint boundary is load-bearing)

The hoisted gathers MUST sit **OUTSIDE** `checkpoint(eig_checkpoint, …)`; the checkpointed
target is a new `_simple_eig_FLmap_cannon` that takes the **already-gathered** slices, so
...
```
leftenv_cannon(ALu_blk, ALd_blk, M, FL_blk, grid; alg):
  @assert grid.N1 == grid.N2                                  # square (FR/AC/ACd maps)
  @assert alg.ifsimple_eig                                    # eigsolve branch bypasses cannon rrules
  ...
  for i in 1:Ni
    ...
    ALu_row = ntuple(j -> cannon_gather_row(ALu_blk[i,j],  grid, l_rs), Nj)   # local-a, full-d
    ALd_col = ntuple(j -> cannon_gather_col(ALd_blk[ir,j], grid, a_rs), Nj)   # full-i, local-l
    λLs, FLs = checkpoint(eig_checkpoint, _simple_eig_FLmap_cannon,
    ...
    for j in 2:Nj                                             # non-leading cycle cells: one sliced step
      ...
      FL_blk'[i,j] = FLmap_cannon_sliced(FL_blk'[i,j-1], ALu_row[j-1], ALd_col[j-1], M[i,j-1], grid; forloop_iter)
      λL[i,j] = λL[i,1];  push!(processed, p)
    end
  end;  return λL, FL_blk'

_simple_eig_FLmap_cannon(FLij_blk, ALu_row, ALd_col, M_i, grid; power_iter, segment_checkpoint, forloop_iter):
  f(v) = (for j in 1:Nj; v = FLmap_cannon_sliced(v, ALu_row[j], ALd_col[j], M_i[j], grid; forloop_iter); end; v)
  simple_eig(f, FLij_blk; power_iter, segment_checkpoint,
             inner_product = (x,y)->cannon_dot(x,y,grid),
             norm_fn       = x   ->cannon_norm(x,grid),
             orth_fn       = v   ->orth_for_ad_cannon(v,grid))     # R1-B3 global ⊥ projection
```
...
- **In the AD loop:** Zygote records. `cannon_gather_row/col` fire once per (i,j) → their
  reduce-scatter adjoints run **once** in the backward. `FLmap_cannon_sliced` fires
...
`Cmap_cannon` gathers FL/FR to **full** (row+col allgather) and returns the full χ×χ
...
```
Cenv_cannon: FL_full[j] = cannon_gather_full(FL_blk[i,jr], grid)   # hoisted, replicated
             FR_full[j] = cannon_gather_full(FR_blk[i,j],  grid)
             f(C) = chain over i of Cmap_cannon_sliced(C, FL_full, FR_full, grid)
...
- C is **replicated** (full χ×χ on every rank), so `simple_eig` uses **plain `dot`/`norm`**
  (every rank holds the identical full C — local == global). No cannon hooks for Cenv.
- The hoisted gather is `cannon_gather_full` (both axes to full), and its **adjoint is
  take-my-block, NOT reduce-scatter** (the replicated-output rule, `Cmap_cannon` rrule
...
- `Cmap_cannon_sliced` rrule returns `(dC_replicated, dFL_full, dFR_full)`;
  `cannon_gather_full`'s rrule turns `dFL_full` → `dFL_blk` by take-my-block.
...
| function | iterate (per-call gather) | fixed boundary (HOISTED once) | cannon map (sliced) | inner_product / norm / orth | output |
|----------|---------------------------|-------------------------------|---------------------|------------------------------|--------|
| `leftenv_cannon`  | FL_blk — **not gathered** (ring block) | ALu_row[j], ALd_col[j] (j=1:Nj) | `FLmap_cannon_sliced` | cannon_dot / cannon_norm / orth_cannon | FL_blk |
| `rightenv_cannon` | FR_blk — col-gather per-call | ARu_g[j], ARd_g[j] (j=1:Nj) | `FRmap_cannon_sliced` | cannon_dot / cannon_norm / orth_cannon | FR_blk |
| `ACenv_cannon`    | AC_blk — row-gather per-call | FL_g[i'], FR_g[i'] (i'=1:Ni, fixed column j) | `ACmap_cannon_sliced` | cannon_dot / cannon_norm / orth_cannon | AC_blk |
| `Cenv_cannon`     | C_full — replicated, not gathered | FL_full[i'], FR_full[i'] (full gather, jr=mod1(j+1,Nj) offset) | `Cmap_cannon_sliced` | dot / norm (replicated; no orth hook) | C_full |
...
- `rightenv_cannon` hoists `ARu_g`, `ARd_g`, **and `FR_g` is the iterate** — wait: in
  `FRmap_cannon_dist(FR_blk, ARu_blk, ARd_blk, M, grid)` the **iterate is FR_blk** and
  ...
  **and FR (col)** — but FR is the iterate, so its gather is **per-call, not hoisted.**
  Hoist only ARu_g (row, full-d) and ARd_g (col, full-i). `FRmap_cannon_sliced` then
...
- Symmetrically `ACenv_cannon`: iterate AC, fixed FL/FR → hoist FL_g (row, full-i) and
...
must split each `*_cannon_dist` into (fixed-operand gathers → hoist) + (iterate gather +
local chain + output RS → per-call `*_cannon_sliced`). For FLmap the iterate FL is **not
...
For each map, the `*_cannon_sliced` rrule is obtained from the `*_cannon_dist` rrule by a
...
3. **Keep the local chain forward** (`_*_cannon_forward_sliced`) and the **type-A output
   reduce-scatter** unchanged.
4. In the pullback: **keep** the type-A output-adjoint allgather of `dresult`, the
   per-l-chunk `chain_backward`/hand-adjoint walk (with the `_CANNON` chains for FR/ACd),
...
5. **Delete the type-B reduce-scatters of the FIXED operands** (`dALu_blk =
   ...
   directly.** The matching reduce-scatter now lives in `cannon_gather_row/col`'s rrule
   and runs once at the hoist boundary.

The result is provably the same gradient (linearity, §5.2). Each sliced rrule keeps the
do-cast scaffolding shape (but mixed precision on the cannon env path is deferred — §11,
...
capture.** A `*_cannon_sliced` could instead accept the slices, and its rrule could still
...
`leftenv_cannon` is plain Julia that calls `cannon_gather_{row,col}` (once) and
`simple_eig` (which calls `FLmap_cannon_sliced` `power_iter[_ad]·Nj` times, and
`cannon_dot`/`cannon_norm`). Zygote differentiates this composition (the status-quo
...
Let `g = cannon_gather_row` (a linear map blk ↦ row-slice) with adjoint `gᵀ =
_cannon_row_reduce_scatter_last`. Un-hoisted (`FLmap_cannon_dist`), one power step is
...
Identical to the un-hoisted path. The same argument covers `cannon_gather_col` and the
Cmap take-my-block adjoint (also linear). This is the formal version of research #5's
"sum-then-scatter ≡ scatter-then-sum."

### 5.3 cannon_dot / cannon_norm rrules

`cannon_dot(x,y,grid) = MPI.Allreduce(dot(x_blk,y_blk), +, grid.comm)` and
`cannon_norm(x,grid) = sqrt(MPI.Allreduce(sum(abs2,x_blk), +, grid.comm))` are
...
```
rrule(cannon_dot)(ds)  = (NoTangent, y_blk·conj(ds), x_blk·ds, NoTangent)            # consumed-as-scalar: NO allreduce
rrule(cannon_norm)(dn) = (NoTangent, x_blk·(real(allreduce(dn,grid.comm))/n), NoTangent)  # denominator: allreduce dn
```
...
**Batch-A finding (refines R1-B2) — `cannon_norm` must allreduce its cotangent; `cannon_dot`
...
isolation). `cannon_norm` is the **normalization denominator**: `simple_eig` does
`v /= cannon_norm(v)`, broadcasting the replicated scalar `n` into a **per-rank division**
...
χ=12 D=2; FP-exact once added). `cannon_dot` is the **inner-product / eigenvalue** hook,
...
`cannon_norm`'s backward is collective on `grid.comm` and rank-uniform. **General rule for
...
`orth_for_ad_cannon(v, grid)` whose rrule is `_dv - cannon_dot(v,_dv,grid)·v`, and give
...
**Precondition:** the cannon env requires `ifsimple_eig=true` (the eigsolve/GMRES branch is
...
2. **Persistent comm buffers for the cannon collectives.** Today every cannon collective
   ...
   shaped views — the cannon helpers use multi-dim `similar(acc)` and non-contiguous leg
   views, so a flat `_comm_sendbuf` is not a drop-in; use flat-buffer + `reshape`, keyed
   per (role, shape). Wire it into `_cannon_row_shift` (the ring recv) and the four
   gather/RS helpers behind a `grid`-scoped pool. **Tag-collision guard (research #12):**
   the cannon collectives use a single fixed tag per type; double-buffering or
   ...
   (the `_cannon_row_shift` `tag` kwarg precedent; 720 band is free).
...
3. **Double-buffered iterate.** `simple_eig` does `v = f(v); v /= norm`. The default
   ...
   path or a cannon-specific `_power_iter_segment` that writes into a held buffer) — it
...
The cannon collectives already have an NCCL fast path (`TENET_USE_NCCL=1`, gated
...
a predicate `_cannon_nccl_worth(χ, D, P)` (threshold calibrated from Part 12: crossover
χ512@16GPU → χ1024@64GPU, i.e. roughly χ·D/√P above a constant) consulted at the cannon
...
- **Pass the `CannonGrid` explicitly** to each env function (research flag: avoid the
  module-global `_cannon_grid_cache` re-derivation; the cache stays as the builder).
- **Block-distributed StructArrays need no type change** (research #10): each `.data`
  ...
  supported). `C` stays replicated (full χ×χ) in `Cenv_cannon`.
...
- **Init scatter is M5** — for M4 tests, build full tensors with a fixed seed and
  `cannon_scatter` to blocks (the existing harness pattern), exactly as the map tests do.

---

## 9. Parity gates + tests (the M4 acceptance criteria)

New file `test/test_cannon_m4.jl` + launcher `test/run_test_cannon_m4.jl` (4-rank,
`mpiexec -n 4`, mirroring `run_test_cannon_m3.jl`). Grids: 2×2 (square; FR/AC/ACd skip
...
FL_ref)`; scatter to blocks, run `leftenv_cannon` → `(λ_c, FL_c_blk)`; `cannon_gather`
→ full; assert `λ_c ≈ λ_ref` (rtol 1e-8) and `FL_full ≈ FL_ref` up to global phase
(rtol 1e-8) — the existing phase-fix idiom (`test_cannon.jl:268-275`). This is the M4
...
assert **bitwise/1e-12** agreement with `K` iterations of the per-call `*_cannon_dist`
map (re-gathering each step). A stale-slice or double-buffer-aliasing bug fails here and
nowhere else.

**Gate 3 — gradient parity (readies M5).** Weighted-sum loss through `leftenv_cannon`
vs serial `leftenv`; compare block cotangents (block-of) at rtol 1e-10; M cotangents
full. Plus a **finite-difference micro-test** of `cannon_dot`/`cannon_norm` rrules.
...
CI note: like the existing cannon tests, M4 tests are launcher-driven (out-of-band of
...
- **Batch A — primitives + FLmap (the template).**
  1. `cannon_gather_row`/`cannon_gather_col` + rrules (reduce-scatter adjoints).
  2. `cannon_dot`/`cannon_norm` rrules.
  3. `FLmap_cannon_sliced` + rrule (surgery from `FLmap_cannon_dist` rrule, §4).
  4. `leftenv_cannon` (General core, hoist ALu/ALd, cannon hooks, no obs/mixed-prec).
  5. Tests: Gate 1+2+3 for leftenv; Gate 4 alloc probe. **Gate before Batch B.**
- **Batch B — rightenv (iterate-gather case).** `FRmap_cannon_sliced` (hoist ARu/ARd,
  per-call FR gather; `FRMAP_LEG5_CANNON_CHAIN` backward) + `rightenv_cannon` + tests.
- **Batch C — ACenv.** `ACmap_cannon_sliced` (hoist FL/FR, per-call AC gather;
  `ACMAP_LEG5_CHAIN`) + `ACenv_cannon` + tests.
- **Batch D — Cenv (replicated-output).** `cannon_gather_full` (take-my-block adjoint) +
  `Cmap_cannon_sliced` + `Cenv_cannon` (plain dot/norm) + the replicated-output gradient
...
- `vumps_step`/`vumps_itr`/`leading_boundary` cannon assembly + the **QR gather seam**
  (`ACCtoALAR` is full-χ per-cell QR; AC must be gathered between `ACenv_cannon` and the
...
- `init_VUMPSRuntime` block scatter (replace the `ifparallel` bcast with `cannon_scatter`).
- Mixed precision (`inner_etype`/`whole_vumps_etype`) on the cannon env path — the
...
3. **`cannon_dot`/`cannon_norm` rrule on the AD path** — confirm the normalization is
