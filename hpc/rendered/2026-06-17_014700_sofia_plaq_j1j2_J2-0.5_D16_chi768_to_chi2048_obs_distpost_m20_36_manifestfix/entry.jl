...
const COMPUTE_XI = getb("COMPUTE_XI", false)
...
function main()
    ...
    say("save_env=$SAVE_ENV compute_xi=$COMPUTE_XI load_folder=$load_folder")
    ...
    CUDA.synchronize()
    MPI.Barrier(COMM)
    say(@sprintf("=== obs distributed-post maxiter20 done in %.1f sec ===", time() - t0))
    say("energy = $(e[1])")
    say("magnetization = $(mag[1])")
    if xi === nothing
        say("correlation_length = skipped")
    else
