...
const CHI_OBS = geti("CHI_OBS", 2048)
const SEED = geti("SEED", 42)
const VUMPS_MAXITER = geti("VUMPS_MAXITER", 2)
...
function main()
    say("=== Slice2D Plaquette J1J2 Square obs TSQR smoke ===")
    say("nprocs=$NPROCS grid=$(NGRID)x$(NGRID) D=$D J2=$J2 seed=$SEED")
    say("load chi=$CHI_LOAD No.$NO_LOAD -> obs chi=$CHI_OBS")
    say("distributed_qr=$DISTRIBUTED_QR maxiter=$VUMPS_MAXITER show_every=$SHOW_EVERY power_iter=$POWER_ITER power_iter_obs=$POWER_ITER_OBS tol=$ENV_TOL forloop=$FORLOOP_ITER")
    say("save_env=$SAVE_ENV folder=$folder")
    ...
    CUDA.synchronize()
    MPI.Barrier(COMM)
    say(@sprintf("=== obs TSQR smoke done in %.1f sec ===", time() - t0))
    say("energy = $(e[1])")
