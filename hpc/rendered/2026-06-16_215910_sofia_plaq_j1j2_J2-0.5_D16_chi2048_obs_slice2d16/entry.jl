...
function main()
    say("=== Slice2D Plaquette J1J2 Square obs ===")
    say("nprocs=$NPROCS grid=$(NGRID)x$(NGRID) D=$D J2=$J2 seed=$SEED")
    say("load chi=$CHI_LOAD No.$NO_LOAD -> obs chi=$CHI_OBS")
    say("VUMPS maxiter=$VUMPS_MAXITER show_every=$SHOW_EVERY power_iter=$POWER_ITER power_iter_obs=$POWER_ITER_OBS tol=$ENV_TOL forloop=$FORLOOP_ITER")
    ...
    CUDA.synchronize()
    MPI.Barrier(COMM)
    say(@sprintf("=== obs done in %.1f sec ===", time() - t0))
    say("energy = $(e[1])")
