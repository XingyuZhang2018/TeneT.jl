...
atype = Array
etype = Float64
D, χ, χshift, maxiter_restart = 2, 8, 1, 100
# pattern = [1 2;
#            2 1]
# pattern = [1 3;
#            2 4]
...
function restriction_ipeps(A)
   ...
   B = Zygote.Buffer(A)
   A[:,:,:,:,:,1] += conj(permutedims(A[:,:,:,:,:,1],(4,2,3,1,5)))
   B[:,:,:,:,:,1] = A[:,:,:,:,:,1]
   B[:,:,:,:,:,2] = permutedims(A[:,:,:,:,:,1],(3,2,1,4,5))
   B[:,:,:,:,:,3] = A[:,:,:,:,:,1]
   B[:,:,:,:,:,4] = permutedims(A[:,:,:,:,:,1],(3,2,1,4,5))
   B[:,:,:,:,:,5] = permutedims(A[:,:,:,:,:,1],(4,2,1,3,5))
   B[:,:,:,:,:,6] = permutedims(A[:,:,:,:,:,1],(4,2,1,3,5))
