# Slice2D (2×2) vs slice FLmap benchmark over the Part-2 D×χ matrix of
...
# NCCL coverage note: slice fwd (allgatherv) + slice/slice2d bwd (allreduce)
# take the NCCL fast path; the Slice2D ring shifts and column reduce-scatter/
# allgather are MPI point-to-point (no NCCL path yet).
#
# The slice2d path is the PRODUCTION form `FLmap_slice2d_dist`: FL, ALu, ALd
...
# rank; slice2d fwd ends with each rank holding only its block (an iterating
# map needs no gather since output distribution = input distribution).
# Likewise slice2d bwd leaves dFL distributed while slice bwd allgathers it.
#
# Launched by Sofia/submit_benchmark_slice2d.sh (mpirun -np 4).
using CUDA, MPI, LinearAlgebra, Zygote, Printf, TeneT
using TeneT: slice2d_grid, slice2d_scatter, FLmap_slice2d_dist, split_ranges
...
report("=== Slice2D $(N1)x$(N2) vs slice FLmap benchmark ($(nprocs) GPU, ", CUDA.name(CUDA.device()), ") ===")
report("methodology: test_MPI_config.jl Part 2 (Float64 leg5, single M, total_splits=$total_splits, nrep=$nrep), slice2d forloop_iter=n per cell (backward-peak formula)")
...
for (D, χ) in vec([(D, χ) for χ in 256:256:1024, D in 8:2:16])
    ...
    ALub = slice2d_scatter(ALu, g)
    ALdb = slice2d_scatter(ALd, g)

    n = pick_n(D, χ)

    slice_fwd_f  = () -> TeneT.FLmap_parallel(FL, ALu, ALd, M; ifparallel = true, forloop_iter)
    slice2d_fwd_f = () -> FLmap_slice2d_dist(blk, ALub, ALdb, M, g; forloop_iter = n)
    slice_bwd_f  = () -> Zygote.pullback(
        x -> sum(TeneT.FLmap_parallel(x, ALu, ALd, M; ifparallel = true, forloop_iter)), FL)[2](1.0)
    slice2d_bwd_f = () -> Zygote.pullback(
        x -> sum(FLmap_slice2d_dist(x, ALub, ALdb, M, g; forloop_iter = n)), blk)[2](1.0)

    t = Dict{String, Float64}()
    for (tag, on) in (("ring", "0"), ("nccl", "1"))
        ENV["TENET_USE_NCCL"] = on
        t["sf_$tag"] = timeit(slice_fwd_f)
        t["cf_$tag"] = timeit(slice2d_fwd_f)
        t["sb_$tag"] = timeit(slice_bwd_f)
        t["cb_$tag"] = timeit(slice2d_bwd_f)
        GC.gc(); CUDA.reclaim()
    end
    ENV["TENET_USE_NCCL"] = "0"

    # block-level parity (no gather needed): slice2d block vs the matching
