# Slice2D MAP timing: FLmap / FRmap / ACmap / ACdmap distributed, over the
...
# single l-chunk → n GEMMs)?". Same methodology as benchmark_slice2d_sofia.jl:
# Float64 leg5 single-M, nrep=3, slice2d forloop_iter = pick_n(D,χ) per cell
...
#
# RING ONLY: the slice2d collectives (row/col allgather, reduce-scatter) are MPI
# point-to-point with no NCCL path (Part 5 fn.2 / Part 10), so ring IS the honest
# slice2d number. The slice path is run UNTIMED, only as the parity reference
# (chunked *_parallel → no serial-ref OOM, unlike Part 9's validator).
#
# Output: per map, slice2d fwd / slice2d bwd (ms, ring) + block parity vs slice.
# Launched by Sofia/submit_benchmark_slice2d_maps.sh (np 4) / _16gpu.sh (np 16).
using CUDA, MPI, LinearAlgebra, Zygote, Printf, TeneT
using TeneT: slice2d_grid, slice2d_scatter, split_ranges,
             FLmap_slice2d_dist, FRmap_slice2d_dist, ACmap_slice2d_dist, ACdmap_slice2d_dist
...
report("=== Slice2D $(N1)x$(N2) MAP timing: FL/FR/AC/ACd ($(nprocs) GPU, ", CUDA.name(CUDA.device()), ") — RING vs NCCL ===")
report("methodology: benchmark_slice2d_sofia.jl (Float64 leg5 single-M, nrep=$nrep, slice2d forloop_iter=n); slice untimed=parity ref; parity recomputed under NCCL (validates the new path)")
report("| map    | D  | χ    | n  | fwd ring | fwd nccl | bwd ring | bwd nccl | parity |")
report("|--------|----|------|----|----------|----------|----------|----------|--------|")

# (name, slice_parallel, slice2d_dist) — arg order is identical for the pair.
const MAPS = (
    ("FLmap",  (a, b, c, M) -> TeneT.FLmap_parallel(a, b, c, M; ifparallel = true, forloop_iter = forloop_slice),
               (a, b, c, M, n) -> FLmap_slice2d_dist(a, b, c, M, g; forloop_iter = n)),
    ("FRmap",  (a, b, c, M) -> TeneT.FRmap_parallel(a, b, c, M; ifparallel = true, forloop_iter = forloop_slice),
               (a, b, c, M, n) -> FRmap_slice2d_dist(a, b, c, M, g; forloop_iter = n)),
    ("ACmap",  (a, b, c, M) -> TeneT.ACmap_parallel(a, b, c, M; ifparallel = true, forloop_iter = forloop_slice),
               (a, b, c, M, n) -> ACmap_slice2d_dist(a, b, c, M, g; forloop_iter = n)),
    ("ACdmap", (a, b, c, M) -> TeneT.ACdmap_parallel(a, b, c, M; ifparallel = true, forloop_iter = forloop_slice),
               (a, b, c, M, n) -> ACdmap_slice2d_dist(a, b, c, M, g; forloop_iter = n)),
)
...
for (D, χ) in vec([(D, χ) for χ in 256:256:1024, D in 8:2:16])
    ...
    for (name, slicef, slice2df) in MAPS
        CUDA.seed!(42)
        ...
        b1 = slice2d_scatter(T1, g); b2 = slice2d_scatter(T2, g); b3 = slice2d_scatter(T3, g)

        slice2d_fwd_f = () -> slice2df(b1, b2, b3, M, n)
        slice2d_bwd_f = () -> Zygote.pullback(x -> sum(slice2df(x, b2, b3, M, n)), b1)[2](1.0)

        tt = Dict{String, Float64}()
        for (tag, on) in (("ring", "0"), ("nccl", "1"))
            ENV["TENET_USE_NCCL"] = on
            tt["f_$tag"] = timeit(slice2d_fwd_f)
            tt["b_$tag"] = timeit(slice2d_bwd_f)
        end

        # parity (UNTIMED) under NCCL — validates the NEW NCCL slice2d path vs the
        # chunked *_parallel slice ref (ring already validated by test_slice2d_m3 +
