# Sofia 4×H200 M3 Cannon validation: distributed parity (forward + Zygote-sum
# gradient) vs the serial kernels for all four M3 cannon-wrapper maps —
# Cmap_cannon / FRmap_cannon_dist / ACmap_cannon_dist / ACdmap_cannon_dist —
# plus per-map device-memory accounting. The NEW value over the 4-rank CPU
# parity gate (test/test_cannon_m3.jl, already green) is the GPU MEMORY check at
...
#
# Launched by Sofia/submit_test_cannon_m3.sh (mpirun -np 4, 2×2 Cannon grid,
# CUDA_VISIBLE_DEVICES pins one GPU per rank). Mirrors test_cannon_sofia.jl's
# mem_line / timed helpers verbatim.
#
# Local CPU smoke (no GPU, tiny dims): runs the SAME *_cannon_dist code on plain
# Arrays under 4 ranks via TENET_BENCH_CPU=1 — confirms the driver parses + runs
# end-to-end before it ever reaches the cluster:
#   TENET_BENCH_CPU=1 julia --project=. examples/MPI_parallel/run_test_cannon_m3_sofia_cpu.jl
...
@assert nprocs == 4 "test_cannon_m3_sofia.jl expects exactly 4 ranks (2×2 grid)"
...
const CHI = parse(Int, get(ENV, "TENET_CANNON_CHI", "400"))   # production χ
const D   = parse(Int, get(ENV, "TENET_CANNON_D",   "10"))    # production D
const CHI_VALID = parse(Int, get(ENV, "TENET_CANNON_CHI_VALID", "256"))
const D_VALID   = parse(Int, get(ENV, "TENET_CANNON_D_VALID",   "8"))
const d_phys = 2
...
# mem_line / timed: verbatim shape from test_cannon_sofia.jl:17-31 (the device
...
const FLOOP = parse(Int, get(ENV, "TENET_CANNON_FLOOP", "4"))
...
function validate_Cmap(χ, D, g; tuple_M)
    ...
    out = Cmap_cannon(C, FLb, FRb, g)                  # full χ×χ replicated
...
# `mapfwd(A,Bu,Bd,M,g)` is the *_cannon_dist; `serial(A,Bu,Bd,M)` is the kernel.
function validate_gather(name, mapfwd, serial, χ, D, g; tuple_M, mem_probe, floop)
    ...
    Ab = cannon_scatter(A, g); Bub = cannon_scatter(Bu, g); Bdb = cannon_scatter(Bd, g)
...
# ── driver ───────────────────────────────────────────────────────────────────
function main()
    g = cannon_grid(2, 2)   # square 2×2 (required by FR/AC/ACd; Cmap accepts it too)
    cells = BENCH_CPU ? [(8, 2)] : [(CHI_VALID, D_VALID), (CHI, D)]

    report("=== M3 Cannon Sofia validation: 2×2 grid, ", _devname(),
    ...
    for (χ, Dc) in cells
        ...
        for tuple_M in (true, false)
            tag = tuple_M ? "" : "/1M"
            f, gd = validate_gather("FRmap$tag", FRmap_cannon_dist, FRmap, χ, Dc, g;
                                    tuple_M, mem_probe = true,  floop)   # MEM load-bearing
            record("FRmap$tag", χ, Dc, f, gd)

            f, gd = validate_gather("ACmap$tag", ACmap_cannon_dist, ACmap, χ, Dc, g;
                                    tuple_M, mem_probe = false, floop)
            record("ACmap$tag", χ, Dc, f, gd)

            f, gd = validate_gather("ACdmap$tag", ACdmap_cannon_dist, ACdmap, χ, Dc, g;
                                    tuple_M, mem_probe = true,  floop)   # MEM load-bearing
