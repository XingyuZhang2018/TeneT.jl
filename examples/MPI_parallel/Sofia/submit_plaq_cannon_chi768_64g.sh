...
# Cannon (block-distributed χ) Plaquette J1J2 Square — D16 χ768 64-GPU (8 nodes × 8 H200, 8×8 grid).
# Same SMOKE as the 16-GPU run (submit_plaq_cannon_chi768.sh, job 1287345) but on 4× the ranks:
...
DRIVER=$GATE/examples/MPI_parallel/J1J2_Square_VUMPS_Plaquette_cannon.jl

CLEAN_LD=$(echo "${LD_LIBRARY_PATH:-}" | tr ':' '\n' | grep -v 'CUDA/12.8.0' | tr '\n' ':' || true)

# Physics / cannon params — overridable from the sbatch environment (--export) for the full run.
...
echo "=== Cannon Plaquette D=${D:-16} J2=${J2:-0.5} χ${CHI_LOAD:-512}→χ${CHI_OPT:-768} OPT_MAXITER=${OPT_MAXITER:-0} VUMPS_MAXITER=${VUMPS_MAXITER:-0} forloop=${FORLOOP_ITER:-16} | 64 GPU 8x8 ==="
