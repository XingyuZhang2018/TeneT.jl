#!/bin/bash -l
#SBATCH --job-name=Sofia_cannon_m3
...
# Sofia 4-GPU M3 Cannon-wrapper validation: distributed parity (forward +
# Zygote-sum gradient) vs the serial kernels for Cmap_cannon / FRmap_cannon_dist
# / ACmap_cannon_dist / ACdmap_cannon_dist, plus per-map device-memory probes
# (esp. FRmap/ACdmap bounded full-i×full-d intermediates via the 2-level chunk).
# Driver: ../test_cannon_m3_sofia.jl
# Plan:   docs/2026-06-13-m3-cannon-wrappers-plan.md (Batch E)
...
mpirun -np 4 -x UCX_MODULE_DIR -x LD_LIBRARY_PATH=$CLEAN_LD -x PATH -x HOME -x JULIA_DEPOT_PATH \
    bash -c "$BASE_ENVS; exec $JULIA --project=../../.. ../test_cannon_m3_sofia.jl"
