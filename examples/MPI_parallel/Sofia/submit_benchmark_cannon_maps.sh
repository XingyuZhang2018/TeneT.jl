#!/bin/bash -l
#SBATCH --job-name=Sofia_cannon_maps
...
# 4-GPU (2×2) cannon MAP timing: FLmap / FRmap / ACmap / ACdmap fwd+bwd (ring)
# over the Part-2 D×χ matrix — measures the gather-class (FR/ACd) vs ring-class
# (FL/AC) cost. Driver: ../benchmark_cannon_maps_sofia.jl. Results → Part 11.
...
export TENET_CANNON_N1=2; \
export TENET_CANNON_N2=2; \
...
echo "=== Sofia 4-GPU (2×2) cannon MAP timing (FL/FR/AC/ACd) ==="
echo "Start: $(date)"
