#!/bin/bash -l
#SBATCH --job-name=Sofia_cannon_m4
...
# Sofia 16-GPU (2-node, 4×4 Cannon grid) M4 Batch-A validation: distributed
# `leftenv_cannon` (env-level Cannon + gather hoisting) forward eigenpair + AD
...
# (incl. the GPU-densify fix in the cannon_gather_row/col rrules). NCCL OFF this
# run (hand ring) to isolate env-layer correctness; a follow-up flips TENET_USE_NCCL=1.
# Driver: ../test_cannon_m4_sofia.jl (grid auto-derived from nprocs: 16 → 4×4).
...
export TENET_CANNON_POWER=40"

echo "=== Sofia 16-GPU (2-node) M4 Batch-A leftenv_cannon validation (4×4) ==="
echo "Start: $(date)"
...
mpirun -np 16 -x UCX_MODULE_DIR -x LD_LIBRARY_PATH=$CLEAN_LD -x PATH -x HOME -x JULIA_DEPOT_PATH \
    bash -c "$BASE_ENVS; exec $JULIA --project=../../.. ../test_cannon_m4_sofia.jl"
echo ""
echo "=== Done: $(date) ==="
