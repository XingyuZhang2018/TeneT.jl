#!/bin/bash
#SBATCH --job-name=mpi_test_2D_4gpu
...
echo "=== test_MPI_config_2D.jl, 4 GPU, $(date) ==="
...
mpirun -np ${MAX_GPU} \
    ...
    bash -c "$ENVS; exec $JULIA --project=$WD/TeneT-2d-validation $WD/TeneT-2d-validation/examples/MPI_parallel/test_MPI_config_2D.jl"
