#!/bin/bash -l
#SBATCH --job-name=Sofia_cannon_m5
...
# Sofia 16-GPU (2-node, 4×4 Cannon grid) M5 validation: the full distributed
# `vumps_step_cannon` (four M4 env solvers + the two QR gather seams + orchestration)
...
# RANDOM rt with power_iter=1 (one map application — serial & cannon run the SAME short
...
# is bit-exact, see test/test_cannon_m5.jl Gate M5-0). NCCL OFF (hand ring) to isolate
# correctness. Driver: ../test_cannon_m5_sofia.jl (grid auto-derived: 16 → 4×4).
...
export TENET_CANNON_CHI=256; \
export TENET_CANNON_D=8; \
export TENET_CANNON_POWER=1"

echo "=== Sofia 16-GPU (2-node) M5 vumps_step_cannon validation (4×4) ==="
echo "Start: $(date)"
...
mpirun -np 16 -x UCX_MODULE_DIR -x LD_LIBRARY_PATH=$CLEAN_LD -x PATH -x HOME -x JULIA_DEPOT_PATH \
    bash -c "$BASE_ENVS; exec $JULIA --project=../../.. ../test_cannon_m5_sofia.jl"
echo ""
echo "=== Done: $(date) ==="
