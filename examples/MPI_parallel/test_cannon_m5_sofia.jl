# Sofia GPU M5 validation: distributed `vumps_step_cannon` (full VUMPS step assembled
...
# the CPU gate (test/test_cannon_m5.jl) is the WHOLE step on CuArray + MPI-CUDA: the
...
#
# Parity uses a RANDOM (non-canonical) rt fed identically to serial and cannon — both run
...
# Design: docs/2026-06-15-m5-vumps-step-cannon-assembly-design.md (§6).
#
# Local CPU smoke (tiny dims, 4 ranks, plain Arrays):
#   TENET_BENCH_CPU=1 julia --project=. examples/MPI_parallel/run_test_cannon_m5_sofia_cpu.jl
...
@assert N * N == nprocs "test_cannon_m5_sofia.jl needs a square nprocs (4→2×2, 16→4×4); got $nprocs"

const CHI = parse(Int, get(ENV, "TENET_CANNON_CHI", "256"))
const D   = parse(Int, get(ENV, "TENET_CANNON_D",   "8"))
const d_phys = 2
const POWER    = parse(Int, get(ENV, "TENET_CANNON_POWER", "20"))
const POWER_AD = 4
...
# ── Gate M5-0: isolated seam parity (identical full input → cannon == serial) ──
function val_seam(χ, Dc, g)
    ...
    ACs = ALCtoAC(AL, C); ACc = gather_sa(ALCtoAC_cannon(scatter_sa(AL, g), C, g), g)
...
# the ~1e-16 cannon-vs-serial residual by cond(C); a random rt's Cenv-output C is
# near-singular (cond ~1e15) → AL diverges O(1) (a fixture-conditioning artifact, not a
# defect — see test/test_cannon_m5.jl Gate M5-1 + the cond(C) confirmation).
function val_step_fwd(χ, Dc, g)
    ...
    rt_c2, e_c = vumps_step_cannon(scatter_rt(rt, g), M, g, alg_c)
    _sync()
    ...
    # pre-seam intermediate AC (vumps_step's first 4 calls), serial vs cannon
    rtb = scatter_rt(rt, g)
    ...
    _, FLc = leftenv_cannon(rtb.AL, conj(rtb.AL), M, rtb.FL, g; alg=alg_c)
    _, FRc = rightenv_cannon(rtb.AR, conj(rtb.AR), M, rtb.FR, g; alg=alg_c)
    _, ACc = ACenv_cannon(AC0c, FLc, M, FRc, g; alg=alg_c)
    _sync()
...
function val_step_grad(χ, Dc, g)
    ...
    WALb=cannon_scatter(WAL,g); WARb=cannon_scatter(WAR,g); WACb=cannon_scatter(WAC,g)
    rs = split_ranges(χ, g.N1); blkof(t) = t[rs[g.r1+1], :, :, rs[g.r2+1]]
    ACb = StructArray([cannon_scatter(AC.data[1], g)], reshape([1],1,1))
    ...
    lal_c(al,c)=real(sum(conj(WACb).*ALCtoAC_cannon(al,c,g).data[1]))
...
function main()
    ...
    report("=== M5 vumps_step_cannon Sofia validation: ", N, "×", N, " grid (", nprocs, " ranks), ",
