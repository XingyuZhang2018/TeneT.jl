# Sofia GPU M4 Batch-A validation: distributed `leftenv_slice2d` (env-level Slice2D
# with gather hoisting) parity vs the SERIAL `leftenv`, on GPU, at multi-card scale.
# The NEW value over the 4-rank CPU gate (test/test_slice2d_m4.jl, already green) is:
...
using TeneT: slice2d_grid, Slice2DGrid, slice2d_scatter, slice2d_gather, split_ranges,
             ...
             ACenv, ACenv_slice2d, Cenv, Cenv_slice2d, VUMPS, General, StructArray
if !BENCH_CPU
    using CUDA
end
...
@assert N * N == nprocs "test_slice2d_m4_sofia.jl needs a square nprocs (4→2×2, 16→4×4); got $nprocs"

const CHI = parse(Int, get(ENV, "TENET_SLICE2D_CHI", "256"))
const D   = parse(Int, get(ENV, "TENET_SLICE2D_D",   "8"))
const d_phys = 2
const POWER = parse(Int, get(ENV, "TENET_SLICE2D_POWER", "40"))   # forward convergence
...
function val_left_fwd(χ, Dc, g)
    ...
    λc, ec = leftenv_slice2d(scatter_sa(ALu, g), scatter_sa(ALd, g), M, scatter_sa(FL, g), g; alg)
...
function val_right_fwd(χ, Dc, g)
    ...
    λc, ec = rightenv_slice2d(scatter_sa(ARu, g), scatter_sa(ARd, g), M, scatter_sa(FR, g), g; alg)
...
function val_ac_fwd(χ, Dc, g)
    ...
    λc, ec = ACenv_slice2d(scatter_sa(AC, g), scatter_sa(FL, g), M, scatter_sa(FR, g), g; alg)
...
function val_c_fwd(χ, Dc, g)
    ...
    λc, ec = Cenv_slice2d(C, scatter_sa(FL, g), scatter_sa(FR, g), g; alg)   # C replicated
...
function val_left_grad(χ, Dc, g)
    ...
    Random.seed!(9101); W = _arr(rand(ComplexF64, χ, Dc, Dc, χ)); Wb = slice2d_scatter(W, g)
    alg = algad()
    lr(au, ad, fl) = real(sum(conj(W)  .* leftenv(au, ad, M, fl; alg)[2].data[1]))
    lc(au, ad, fl) = real(sum(conj(Wb) .* leftenv_slice2d(au, ad, M, fl, g; alg)[2].data[1]))
...
function val_right_grad(χ, Dc, g)
    ...
    Random.seed!(9201); W = _arr(rand(ComplexF64, χ, Dc, Dc, χ)); Wb = slice2d_scatter(W, g)
    alg = algad()
    lr(au, ad, fr) = real(sum(conj(W)  .* rightenv(au, ad, M, fr; alg)[2].data[1]))
    lc(au, ad, fr) = real(sum(conj(Wb) .* rightenv_slice2d(au, ad, M, fr, g; alg)[2].data[1]))
...
function val_ac_grad(χ, Dc, g)
    ...
    Random.seed!(9301); W = _arr(rand(ComplexF64, χ, Dc, Dc, χ)); Wb = slice2d_scatter(W, g)
    alg = algad()
    lr(ac, fl, fr) = real(sum(conj(W)  .* ACenv(ac, fl, M, fr; alg)[2].data[1]))
    lc(ac, fl, fr) = real(sum(conj(Wb) .* ACenv_slice2d(ac, fl, M, fr, g; alg)[2].data[1]))
...
function val_c_grad(χ, Dc, g)
    ...
    lc(c, fl, fr) = real(sum(conj(W) .* Cenv_slice2d(c, fl, fr, g; alg)[2].data[1]))
