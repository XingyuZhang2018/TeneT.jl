# Plaquette Cannon distributed_qr observable write smoke (4 ranks, 2x2 grid, CPU).
...
using TeneT: J1J2, Square, VUMPS, Plaquette, GradientOptimize,
             cannon_grid, observable

MPI.Init()
const comm = MPI.COMM_WORLD
const rank = MPI.Comm_rank(comm)
@assert MPI.Comm_size(comm) == 4 "test_cannon_plaq_tsqr_observable_write.jl expects exactly 4 ranks"
...
@testset "Plaquette distributed_qr observable writes log" begin
    ...
    alg = VUMPS{Plaquette{Square}}(grid=cannon_grid(2, 2),
                                   distributed_qr=true,
                                   ifsimple_eig=true,
                                   ifparallel=false,
                                   forloop_iter=1,
