...
# Run: julia --project=. test/run_test_cannon_energy_plaq.jl
#
# Serial energy_value (grid=nothing, full env) vs cannon energy_value (grid set, BLOCK env
# from the no-gather ObsEnv path, contractions via FLmap/ACmap/FRmap/ACdmap_cannon_dist +
# cannon_dot). Same random iPEPS A + boundary runtime; compares the per-bond e_dict and the
# total energy. J1H/J1V/diag(imag) kernels are exact → machine precision; J2 (oc_22) carries
# the ~1e-10 random-Q0 noise floor (serial AND cannon each have an independent realization),
...
using TeneT: cannon_grid, cannon_scatter, VUMPS, Plaquette, Square, J1J2, Heisenberg, StructArray,
...
@assert MPI.Comm_size(comm) == 4 "test_cannon_energy_plaq.jl expects exactly 4 ranks"
...
# max |serial - cannon| over a bond-energy sub-dict
...
@testset "Gate M6: distributed energy_value parity (Plaquette J1J2 Square)" begin
    ...
    eTot = abs(E_s - E_c)
    say(@sprintf("  [M6 energy] E_serial=%.12g%+.3eim  E_cannon=%.12g%+.3eim",
    ...
    say(@sprintf("  [M6 imag_error] serial=%.12g cannon=%.12g diff=%.2e", ie_s, ie_c, abs(ie_s-ie_c)))
    @test abs(ie_s - ie_c) ≤ 1e-9
end

# ── Gate M6-grad: gradient parity dE/dA (the optimization-critical check) ──────
# energy_value is differentiated in energy(); the cannon contraction's A-gradient must match
...
#      which directly validates the FLmap/ACmap_cannon_dist + cannon_dot backward rrules.
...
#      poorly-converged obs env, amplifying both cannon's FP-reassociation and serial's Q0
#      jitter to the same floor. So the rigorous statement is: cannon adds no more gradient
#      error than re-running serial with a fresh Q0 (cannon-vs-serial ≤ serial-vs-serial).
...
@testset "Gate M6-grad: per-kernel dE/dA (deterministic kernels exact)" begin
    g = cannon_grid(2, 2); χ, D, d = 14, 2, 2
    Random.seed!(8800)
    mk(dims...) = rand(ComplexF64, dims...); sc(t) = cannon_scatter(t, g)
    ...
    # 5e-4 of the ill-conditioned obs env), so this is a DISCRIMINATING check — cannon must track
    # serial's inherent Q0 noise down here. Assert cannon-vs-serial ≤ 2× serial-vs-serial (fresh
    # Q0), which a fixed-size cannon bug ≳1e-7 could not satisfy at this low floor.
    ...
    g22  = relg(g22s, g22c)        # cannon vs serial (independent Q0)
    g22n = relg(g22s, g22s2)       # serial vs serial (independent Q0) — the inherent floor
    say(@sprintf("  [M6-grad kernels] oc_22 cannon-vs-serial=%.2e  serial-vs-serial=%.2e", g22, g22n))
    @test g22 ≤ max(2 * g22n, 1e-7)
end

@testset "Gate M6-grad: full dE/dA — cannon ≤ serial's inherent Q0 noise" begin
    g = cannon_grid(2, 2)
    χ, D, d = 14, 2, 2
    pat = [1 3; 2 4]; nu = 4
    Random.seed!(7711)
    ...
    say(@sprintf("  [M6-grad full] serial-vs-cannon = %.2e   serial-vs-serial (fresh Q0) = %.2e", rel, rel_ss))
    @test rel ≤ max(3 * rel_ss, 1e-7)   # cannon no worse than serial's own Q0 jitter
end
...
# energy_value — e.g. Honeycomb's oc_13, Heisenberg — is not cannon-ized). Block χ = χ/N1; a
...
@testset "Gate M6-scope: ObsEnv no-gather is J1J2{Square}-only" begin
    g = cannon_grid(2, 2); χ, D, d = 14, 2, 2; pat = [1 3; 2 4]; nu = 4
    Random.seed!(7733)
    sa(dims) = StructArray([rand(ComplexF64, dims...) for _ in 1:nu], pat)
    A  = sa((D, D, D, D, d))
    rt = PlaquetteVUMPSRuntime(sa((χ,D,D,χ)), sa((χ,χ)), sa((χ,D,D,χ)))
    ...
    # Heisenberg{Square}: NOT cannon-ized → must gather to FULL (FLo first leg = χ)
...
say("all cannon distributed-energy gates done.")
