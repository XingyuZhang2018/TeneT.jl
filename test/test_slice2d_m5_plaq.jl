# M5-plaq: Plaquette-mode slice2d vumps_step parity (4 ranks, 2×2 grid, CPU).
# Run via: julia --project=. test/run_test_slice2d_m5_plaq.jl
#
# Plaquette slice2d mirrors General (M5) with FR←FL[:,jr] and no AR/rightenv. Same
...
# qrpos amplifies the 1e-16 residual by cond(C); see test_slice2d_m5.jl.)
...
using TeneT: slice2d_grid, Slice2DGrid, slice2d_scatter, slice2d_gather, split_ranges,
             ...
             ALCtoAC, ALCtoAC_slice2d, ACCtoAL, ACCtoAL_slice2d_gather_ref, ACCtoAL_tsqr_slice2d,
             leftenv, leftenv_slice2d, ACenv_plaq, ACenv_plaq_slice2d, Cenv_plaq, Cenv_plaq_slice2d, qrpos
import ChainRulesCore

MPI.Init()
const comm = MPI.COMM_WORLD
const rank = MPI.Comm_rank(comm)
@assert MPI.Comm_size(comm) == 4 "test_slice2d_m5_plaq.jl expects exactly 4 ranks"
...
# ── Gate M5p-0: ACCtoAL_slice2d seam bit-parity (identical input) ──────────────
@testset "Gate M5p-0: ACCtoAL_slice2d bit-parity" begin
    g = slice2d_grid(2, 2)
    for (ci, pat) in enumerate(PATS)
        ...
        ALc, eLc = ACCtoAL_slice2d_gather_ref(scatter_sa(AC, g), C, g)
...
@testset "Gate M5p-1: vumps_step_slice2d(Plaquette) forward parity" begin
    g = slice2d_grid(2, 2)
    for (ci, pat) in enumerate(PATS)
        χ, D = 14, 2
        ...
        AC0c = ALCtoAC_slice2d(rtb.AL, rtb.C, g); _, FLc2 = leftenv_slice2d(rtb.AL, conj(rtb.AL), M, rtb.FL, g; alg=alg_c)
        _, ACc = ACenv_plaq_slice2d(AC0c, FLc2, M, g; alg=alg_c)
...
@testset "Gate M5p-2: ACCtoAL_slice2d gradient parity" begin
    g = slice2d_grid(2, 2)
    χ, D = 10, 2
    ...
    for (ci, pat) in enumerate(PATS)
        ...
        Random.seed!(850 + ci)
        WAL = [rand(ComplexF64, χ, D, D, χ) for _ in 1:nu]; WALb = [slice2d_scatter(W, g) for W in WAL]
        ACb = scatter_sa(AC, g)
        lr(ac, c) = let (al, _) = ACCtoAL(ac, c);        real(sum(sum(conj(WAL[k])  .* al.data[k]) for k in 1:nu)) end
        lc(ac, c) = let (al, _) = ACCtoAL_tsqr_slice2d(ac, c, g); real(sum(sum(conj(WALb[k]) .* al.data[k]) for k in 1:nu)) end
...
# ── Gate M5p-3: init_VUMPSRuntime_slice2d(Plaquette) cross-rank consistency ─────
@testset "Gate M5p-3: init(Plaquette) cross-rank consistency" begin
    ...
    rt = init_VUMPSRuntime_slice2d(M, χ, g, algp(g))
...
@testset "Gate M5p-4: vumps_step(Plaquette) routes to slice2d" begin
    src = slice2d_source()
    @test occursin("ACCtoAL_slice2d(AC_blk, C, grid::Slice2DGrid) = ACCtoAL_tsqr_slice2d", src)
    @test !occursin("ACCtoAL_slice2d(AC_blk, C, grid::Slice2DGrid) = ACCtoAL_slice2d_gather_ref", src)
    g = slice2d_grid(2, 2)
    rt, M = build_rt(14, 2; seed=9500, pat=[1 3; 2 4])
    alg_c = algp(g); alg_c.power_iter = 15
    rtb = scatter_rt(rt, g)
    via, e1 = vumps_step(rtb, M, alg_c)                       # routes to slice2d
