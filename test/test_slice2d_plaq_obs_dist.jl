# Plaquette Slice2D distributed observable post-measurement smoke.
# Run via: julia --project=. test/run_test_slice2d_plaq_obs_dist.jl
using Test, MPI, LinearAlgebra, Random, Printf
using TeneT
using TeneT: slice2d_grid, slice2d_scatter, VUMPS, Plaquette, Square, J1J2,
...
@assert MPI.Comm_size(comm) == 4 "test_slice2d_plaq_obs_dist.jl expects exactly 4 ranks"
...
@testset "Plaquette Slice2D mag/xi run on block env" begin
    g = slice2d_grid(2, 2)
    chi, D, d = 8, 2, 2
    pat = [1 3; 2 4]
    ...
    if rank == 0
        say(@sprintf("  mag serial=%.12g slice2d=%.12g", real(mag_s[1]), real(mag_c[1])))
        say(@sprintf("  xi  serial=%.12g slice2d=%.12g", real(xi_s), real(xi_c)))
        @test relerr(real(mag_c[1]), real(mag_s[1])) <= 1e-10
        @test relerr(real(xi_c), real(xi_s)) <= 1e-8
    end
end

rank == 0 && println("Plaquette Slice2D distributed observable post-measurement smoke done.")
