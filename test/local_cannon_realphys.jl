# Local real-physics validation of the distributed (cannon) Plaquette energy + full opt loop.
# Run: julia --project=. test/run_local_cannon_realphys.jl   (4 ranks, 2×2 grid, CPU)
#
# Phase 1 — converged-A energy: load J2=0.5 D2 χ23 No.10 (history.log converged E=-0.470392432614),
#   serial vs cannon (2×2) leading_boundary + block ObsEnv + DISTRIBUTED energy_value. Both must
#   reproduce the history energy; cannon must match serial to machine precision (real-physics M6).
#   (NB: -0.4704 is the Plaquette single-env value; -0.4713 was the General-updown estimate.)
# Phase 2 — random-init cannon optimization: optimise_ipeps with grid set, random A, a few LBFGS
...
using TeneT: J1J2, Square, VUMPS, Plaquette, GradientOptimize, init_ipeps, optimise_ipeps,
             cannon_grid, build_A, initialize_env, ObsEnv, energy_value, leading_boundary
MPI.Init()
const comm   = MPI.COMM_WORLD
const rank   = MPI.Comm_rank(comm)
const nprocs = MPI.Comm_size(comm)
@assert nprocs == 4 "local_cannon_realphys.jl expects 4 ranks (2×2 grid)"
...
say("=== Phase 1: converged A (J2=0.5 D2 χ23 No.10) energy — serial vs cannon (2×2) ===")
es, errs = phys_energy(nothing)
say(@sprintf("  SERIAL Plaquette : energy = %.12f   (vumps err %.2e)", es, errs))
ec, errc = phys_energy(cannon_grid(N, N))
say(@sprintf("  CANNON Plaquette : energy = %.12f   (vumps err %.2e)", ec, errc))
say(@sprintf("  Δ(serial,cannon) = %.2e    history.log χ23 = -0.470392432614", abs(es - ec)))

# ── Phase 2: random-init cannon optimization (distributed energy + observable fix) ────
say("=== Phase 2: random-init cannon optimization (3 LBFGS iters, distributed energy + observable) ===")
Random.seed!(2024)   # identical RNG on every rank → replicated random A (cannon needs A replicated)
const TMP = joinpath(pkgdir(TeneT), "data", "_local_cannon_smoketest",
                     "$model", "$pattern", "VUMPS_Plaquette", "Float64", "seed2024")
pc = mkparams(cannon_grid(N, N), TMP; maxit=3)
...
say("=== local cannon real-physics validation done ===")
