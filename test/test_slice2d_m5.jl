# M5: slice2d vumps_step assembly + the QR gather seam (4 ranks, 2×2 grid, CPU).
# Run via: julia --project=. test/run_test_slice2d_m5.jl
# Design: docs/2026-06-15-m5-vumps-step-slice2d-assembly-design.md
#
# NOTE: the parity gates feed serial and slice2d the SAME (random, non-canonical) rt and
...
#   M5-0  isolated seam BIT-PARITY: feed IDENTICAL full AC/C → ACCtoALAR_slice2d_gather_ref/ALCtoAC_slice2d
#         bit-match serial (no gauge ambiguity — same input). The rigorous seam proof.
#   M5-1  full-step forward parity: vumps_step_slice2d vs serial. FL/FR/C/err at 1e-8;
...
#         Plain + Recompute.
#   M5-3  init_VUMPSRuntime_slice2d cross-rank consistency (catches the un-bcast'd RNG bug, F1).
#   M5-4  routing: vumps_step(rt, M, alg{grid}) == vumps_step_slice2d directly.
using Test
using MPI
using LinearAlgebra
using Random
...
using TeneT: slice2d_grid, Slice2DGrid, slice2d_scatter, slice2d_gather, split_ranges,
             ...
             vumps_step, init_VUMPSRuntime_slice2d, vumps_step_slice2d,
             ALCtoAC, ALCtoAC_slice2d, ACCtoALAR, ACCtoALAR_dist_slice2d,
             leftenv, leftenv_slice2d, rightenv, rightenv_slice2d,
             ACenv, ACenv_slice2d, Cenv, Cenv_slice2d, checkpoint, qrpos,
             ALCtoAC_slice2d_gather_ref, ACCtoALAR_slice2d_gather_ref
import ChainRulesCore

MPI.Init()
const comm = MPI.COMM_WORLD
const rank = MPI.Comm_rank(comm)
@assert MPI.Comm_size(comm) == 4 "test_slice2d_m5.jl expects exactly 4 ranks"
...
@testset "Gate M5-0: seam bit-parity (identical input)" begin
    ...
    for (ci, (Ni, Nj, pat)) in enumerate([(1, 1, nothing), (2, 2, [1 2; 2 1]), (2, 2, nothing)])
        ...
        # ACCtoALAR: identical full AC/C → slice2d (gathered) must equal serial bit-for-bit.
        ...
        # ALCtoAC: identical full AL/C → slice2d (gathered) must equal serial bit-for-bit.
        ACs = ALCtoAC(AL, C)
        ACcf = gather_sa(ALCtoAC_slice2d_gather_ref(scatter_sa(AL, g), C, g), g)
        for idx in 1:nu
...
# Validates the assembled vumps_step_slice2d via the STABLE, conditioning-independent
...
# amplifies the inevitable ~1e-16 slice2d-vs-serial residual by cond(C). A random,
...
@testset "Gate M5-1: vumps_step_slice2d forward parity" begin
    g = slice2d_grid(2, 2)
    for (ci, (Ni, Nj, pat)) in enumerate([(1, 1, nothing), (2, 2, [1 2; 2 1]), (2, 2, nothing)])
        χ, D = 14, 2
        ...
        rt_c2, err_c = vumps_step_slice2d(rtb, M, g, alg_c)
        ...
        _, FLc2 = leftenv_slice2d(rtb.AL, conj(rtb.AL), M, rtb.FL, g; alg=alg_c)
        _, FRc2 = rightenv_slice2d(rtb.AR, conj(rtb.AR), M, rtb.FR, g; alg=alg_c)
        _, ACc = ACenv_slice2d(AC0c, FLc2, M, FRc2, g; alg=alg_c)
...
@testset "Gate M5-2: seam gradient parity (R-1 catcher)" begin
    ...
    for (ci, (Ni, Nj, pat)) in enumerate([(1, 1, nothing), (2, 2, [1 2; 2 1])])
        ...
        WALb = [slice2d_scatter(W, g) for W in WAL]; WARb = [slice2d_scatter(W, g) for W in WAR]
        WACb = [slice2d_scatter(W, g) for W in WAC]
        ...
        lqr_c(ac, c)   = let (al, ar, _, _) = ACCtoALAR_dist_slice2d(ac, c, g)
        ...
        lqr_cr(ac, c) = let (al, ar, _, _) = checkpoint(Recompute(), ACCtoALAR_dist_slice2d, ac, c, g)
            real(sum(sum(conj(WALb[k]) .* al.data[k]) + sum(conj(WARb[k]) .* ar.data[k]) for k in 1:nu)) end
        ...
        lal_c(al, c)   = real(sum(sum(conj(WACb[k]) .* ALCtoAC_slice2d(al, c, g).data[k]) for k in 1:nu))
...
# rt.AL is consumed TWICE in one step: by the entry seam ALCtoAC_slice2d (gather adjoint =
# take-my-block) AND by leftenv_slice2d (gather adjoint = reduce-scatter). Its cotangent must
...
@testset "Gate M5-2b: cross-seam cotangent sum (dAL/dAR)" begin
    ...
    for (ci, (Ni, Nj, pat)) in enumerate([(1, 1, nothing), (2, 2, [1 2; 2 1])])
        ...
        Random.seed!(8850 + ci)
        W  = [rand(ComplexF64, χ, D, D, χ) for _ in 1:nu]; Wb = [slice2d_scatter(w, g) for w in W]
        rtb = scatter_rt(rt, g)
        ...
        lc(al, ar, c, fl, fr) = let ac = ALCtoAC_slice2d(al, c, g)
            _, FLc = leftenv_slice2d(al, conj(al), M, fl, g; alg=alg_c); _, FRc = rightenv_slice2d(ar, conj(ar), M, fr, g; alg=alg_c)
            _, AC = ACenv_slice2d(ac, FLc, M, FRc, g; alg=alg_c); real(sum(sum(conj(Wb[k]) .* AC.data[k]) for k in 1:nu)) end
...
# ── Gate M5-3: init_VUMPSRuntime_slice2d cross-rank consistency (F1 catcher) ────
@testset "Gate M5-3: init_VUMPSRuntime_slice2d cross-rank consistency" begin
    g = slice2d_grid(2, 2)
    χ, D = 8, 2
    Random.seed!(9000)
    ...
    # in init_VUMPSRuntime_slice2d load-bearing: with it, the gathered runtime is bitwise-identical
    ...
    rt = init_VUMPSRuntime_slice2d(M, χ, g, alg_c)
...
@testset "Gate M5-4: vumps_step routes to slice2d when grid set" begin
    g = slice2d_grid(2, 2)
    rt, M = build_rt(2, 2, 14, 2; seed=9500)
    alg_c = VUMPS(General(); power_iter=15, forloop_iter=1, grid=g, ALG_KW...)
    rtb = scatter_rt(rt, g)
    via_routing, e1 = vumps_step(rtb, M, alg_c)               # routes to slice2d
    direct, e2      = vumps_step_slice2d(rtb, M, g, alg_c)
    @test e1 ≈ e2
