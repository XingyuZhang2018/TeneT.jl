# slice2d ObsEnv parity (4 ranks, 2×2 grid, CPU). Run: julia --project=. test/run_test_slice2d_obsenv.jl
#
# Validates the distributed observation environment: serial ObsEnv vs slice2d ObsEnv (which
# computes block obs envs via leftenv/rightenv_slice2d ifobs=true, then gathers to full).
...
# (The end-to-end physical check — slice2d energy == serial == -0.4713 on a converged J1J2 A —
...
using TeneT: slice2d_grid, slice2d_scatter, slice2d_gather, VUMPS, General, Plaquette, Square,
             StructArray, VUMPSRuntime, PlaquetteVUMPSRuntime, ObsEnv
MPI.Init()
const comm = MPI.COMM_WORLD
const rank = MPI.Comm_rank(comm)
@assert MPI.Comm_size(comm) == 4 "test_slice2d_obsenv.jl expects exactly 4 ranks"
...
@testset "Gate M5o-1: single-env ObsEnv parity" begin
    ...
    for (ci, (Ni, Nj, pat)) in enumerate([(1, 1, nothing), (2, 2, [1 3; 2 4])])
        ...
        env_c = ObsEnv(scatter_rt(rt, g), M, algp(g), nothing)   # slice2d → gathered full VUMPSEnv
...
@testset "Gate M5o-3: Plaquette ObsEnv parity" begin
    ...
    env_s = ObsEnv(rt, M, alg_s, nothing)
    env_c = ObsEnv(rtb, M, alg_c, nothing)   # slice2d → gathered full PlaquetteVUMPSEnv
...
say("all slice2d-ObsEnv gates done.")
