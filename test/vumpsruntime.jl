...
@testset "$(Ni)x$(Nj) VUMPSRuntime with $(symmetry) $atype{$dtype}" for Ni = [2], Nj = [2], atype = [Array], dtype = [ComplexF64], symmetry in [:none]
...
@testset "$(Ni)x$(Nj) obs_env with $(symmetry) $atype{$dtype}" for Ni = [2], Nj = [2], atype = [Array], dtype = [ComplexF64], symmetry in [:none]
    d = 4
    M = [randinitial(Val(symmetry), atype, dtype, d, d, d, d; dir = [-1,1,1,-1]) for i in 1:Ni, j in 1:Nj]
