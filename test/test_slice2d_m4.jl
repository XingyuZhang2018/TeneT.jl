# M4 Batches A–D: distributed leftenv/rightenv/ACenv/Cenv _slice2d (gather hoisting).
# Run via: julia --project=. test/run_test_slice2d_m4.jl   (spawns 4 ranks)
...
using TeneT: slice2d_grid, Slice2DGrid, slice2d_scatter, slice2d_gather,
             slice2d_dot, slice2d_norm, slice2d_gather_row, slice2d_gather_col,
             orth_for_ad, orth_for_ad_slice2d, split_ranges,
             FLmap_slice2d_dist, FLmap_slice2d_sliced,
             FRmap_slice2d_dist, FRmap_slice2d_sliced,
             ACmap_slice2d_dist, ACmap_slice2d_sliced,
             leftenv, leftenv_slice2d, rightenv, rightenv_slice2d,
             ACenv, ACenv_slice2d, Cenv, Cenv_slice2d,
...
@assert MPI.Comm_size(comm) == 4 "test_slice2d_m4.jl expects exactly 4 ranks"
...
function flmap_sliced_rrule_body()
    ...
    sig = "function ChainRulesCore.rrule(::typeof(FLmap_slice2d_sliced)"
    start = findfirst(sig, src)
    @test start !== nothing
    tail = src[last(start):end]
...
# global-phase-insensitive eigenvector comparison (test_slice2d.jl idiom)
...
@testset "Gate 0: FLmap sliced uses row allgather for FL iterate" begin
    body = slice2d_forward_sliced_body()
    @test occursin("FL_row = slice2d_gather_row", body)
    @test occursin("ALd_chunk = view(ALd_col, :, :, :, ch)", body)
    @test occursin("Hc = _slice2d_stage1(FL_row, ALd_chunk)", body)
    @test !occursin("_slice2d_row_shift", body)
...
@testset "Gate 1: leftenv_slice2d eigenpair parity" begin
    g = slice2d_grid(2, 2)
    ...
    for (ci, (Ni, Nj, pat)) in enumerate(cases), χ in (16, 18)
        ...
        λc, FLc = leftenv_slice2d(ALu_b, ALd_b, M, FL_b, g; alg)      # distributed, hoisted
...
# Bitwise-ish: both call _slice2d_forward_sliced with the SAME gathered slices, so
...
@testset "Gate 2: hoisting equivalence (sliced == dist)" begin
    ...
    FLb, ALub, ALdb = slice2d_scatter(FL, g), slice2d_scatter(ALu, g), slice2d_scatter(ALd, g)
    χg = MPI.Allreduce(size(ALub, 1), +, g.col_comm)
    a_rs = split_ranges(χg, g.N1); l_rs = split_ranges(χg, g.N2)
    ALu_row = slice2d_gather_row(ALub, g, l_rs)
    ALd_col = slice2d_gather_col(ALdb, g, a_rs)
    # single application: sliced (pre-gathered) == dist (gathers internally)
    w_dist   = FLmap_slice2d_dist(FLb, ALub, ALdb, M, g)
    w_sliced = FLmap_slice2d_sliced(FLb, ALu_row, ALd_col, M, g)
    @test w_sliced ≈ w_dist rtol=1e-13
    # K iterations with the hoisted slices held FIXED (the leftenv power loop)
    v_d, v_s = FLb, FLb
    for _ in 1:5
        v_d = FLmap_slice2d_dist(v_d, ALub, ALdb, M, g)
        v_s = FLmap_slice2d_sliced(v_s, ALu_row, ALd_col, M, g)
    end
    ...
    # dist gradient exactly — pins sliced-rrule + slice2d_gather_row/col rrules == dist rrule
    # under AD, before Batch E introduces buffer reuse (R1-M8).
    Random.seed!(7001); Wb2 = slice2d_scatter(rand(ComplexF64, χ, D, D, χ), g)
    ld(fl, au, ad) = real(sum(conj(Wb2) .* FLmap_slice2d_dist(fl, au, ad, M, g)))
    lh(fl, au, ad) = real(sum(conj(Wb2) .* FLmap_slice2d_sliced(fl,
                          slice2d_gather_row(au, g, l_rs), slice2d_gather_col(ad, g, a_rs), M, g)))
...
# conj / real-projection bugs). Compares slice2d_dot/slice2d_norm/orth_for_ad_slice2d
# (on blocks) to dot/norm/orth_for_ad on the FULL tensor, block-restricted.
@testset "Gate 3a: slice2d_dot/norm/orth rrules (complex)" begin
    g = slice2d_grid(2, 2)
    χ, D = 16, 3
    Random.seed!(8000)
    ...
    xb, yb, dvb = slice2d_scatter(x, g), slice2d_scatter(y, g), slice2d_scatter(dv, g)

    # slice2d_dot: dx = y·conj(ds), dy = x·ds  (ChainRules dot convention; ds complex)
    ds = 0.6 + 0.5im
    s, dot_back = ChainRulesCore.rrule(slice2d_dot, xb, yb, g)
    ...
    # slice2d_norm (normalization denominator): the incoming `dn` is each rank's PIECE
    ...
    n, norm_back = ChainRulesCore.rrule(slice2d_norm, xb, g)
    @test n ≈ norm(x) rtol=1e-12
    _, dx_n, _ = norm_back(dn_r)
    @test dx_n ≈ blkof(x .* (real(dn_sum) / norm(x))) rtol=1e-12

    # orth_for_ad_slice2d: dv - slice2d_dot(v,dv)·v  == blkof of serial orth_for_ad
    _, oc_back = ChainRulesCore.rrule(orth_for_ad_slice2d, xb, g)
...
# ── Gate 3b: leftenv_slice2d gradient parity vs serial (in-situ, all rrules) ───
# Per-rank weighted-sum loss on the block-distributed output (disjoint tiles), the
# established slice2d-gradient pattern. Validates the WHOLE AD chain in composition:
# gather wrappers + sliced-map rrule + slice2d_dot/norm/orth rrules. Small power_iter
# (gradient parity needs the SAME finite computation, not convergence).
@testset "Gate 3b: leftenv_slice2d gradient parity" begin
    g = slice2d_grid(2, 2)
    χ, D = 12, 2
    a_rs = split_ranges(χ, g.N1); i_rs = split_ranges(χ, g.N2)
    blkof(t) = t[a_rs[g.r1+1], :, :, i_rs[g.r2+1]]
    ...
    for (ci, (Ni, Nj, pat, mode)) in enumerate(configs)
        ...
        Wbs = [slice2d_scatter(W, g) for W in Ws]
        alg = mode === :recompute ?
        ...
        loss_c(au, ad, fl)   = real(sum(sum(conj(Wbs[c]) .* leftenv_slice2d(au, ad, M, fl, g; alg)[2].data[c]) for c in 1:nu))
...
# ── Batch B: rightenv_slice2d ─────────────────────────────────────────────────
@testset "Gate B1: rightenv_slice2d eigenpair parity" begin
    g = slice2d_grid(2, 2)
    cases = [(1, 1, nothing), (1, 2, nothing), (2, 2, nothing), (2, 2, [1 2; 2 1])]
    for (ci, (Ni, Nj, pat)) in enumerate(cases), χ in (16, 18)
        D = 3
        ...
        λc, FRc = rightenv_slice2d(ARu_b, ARd_b, M, FR_b, g; alg)
...
@testset "Gate B2: FRmap hoisting equivalence (sliced == dist)" begin
    g = slice2d_grid(2, 2); χ, D = 16, 3
    Random.seed!(5700)
    FR  = rand(ComplexF64, χ, D, D, χ); ARu = rand(ComplexF64, χ, D, D, χ); ARd = rand(ComplexF64, χ, D, D, χ)
    M   = rand(ComplexF64, D, D, D, D, 2)
    FRb, ARub, ARdb = slice2d_scatter(FR, g), slice2d_scatter(ARu, g), slice2d_scatter(ARd, g)
    χg = MPI.Allreduce(size(ARdb, 1), +, g.col_comm); p_rs = split_ranges(χg, g.N1)
    ARu_g = slice2d_gather_row(ARub, g, p_rs); ARd_g = slice2d_gather_col(ARdb, g, p_rs)
    @test FRmap_slice2d_sliced(FRb, ARu_g, ARd_g, M, g) ≈ FRmap_slice2d_dist(FRb, ARub, ARdb, M, g) rtol=1e-13
    v_d, v_s = FRb, FRb
    for _ in 1:5
        v_d = FRmap_slice2d_dist(v_d, ARub, ARdb, M, g)
        v_s = FRmap_slice2d_sliced(v_s, ARu_g, ARd_g, M, g)
    end
    @test v_s ≈ v_d rtol=1e-12
    Random.seed!(5701); Wb = slice2d_scatter(rand(ComplexF64, χ, D, D, χ), g)
    ld(fr, au, ad) = real(sum(conj(Wb) .* FRmap_slice2d_dist(fr, au, ad, M, g)))
    lh(fr, au, ad) = real(sum(conj(Wb) .* FRmap_slice2d_sliced(fr, slice2d_gather_row(au, g, p_rs), slice2d_gather_col(ad, g, p_rs), M, g)))
    gd = Zygote.gradient(ld, FRb, ARub, ARdb); gh = Zygote.gradient(lh, FRb, ARub, ARdb)
    for k in 1:3; @test gh[k] ≈ gd[k] rtol=1e-10; end
end

@testset "Gate B3: rightenv_slice2d gradient parity" begin
    g = slice2d_grid(2, 2); χ, D = 12, 2
    a_rs = split_ranges(χ, g.N1); i_rs = split_ranges(χ, g.N2)
    blkof(t) = t[a_rs[g.r1+1], :, :, i_rs[g.r2+1]]
    ...
    for (ci, (Ni, Nj, pat, mode)) in enumerate(configs)
        ...
        loss_c(au, ad, fr)   = real(sum(sum(conj(Wbs[c]) .* rightenv_slice2d(au, ad, M, fr, g; alg)[2].data[c]) for c in 1:nu))
...
# ── Batch C: ACenv_slice2d (axis-transposed; non-leading cells normalize) ─────
@testset "Gate C1: ACenv_slice2d eigenpair parity" begin
    g = slice2d_grid(2, 2)
    # include Ni≠Nj (2×1, 1×2) to exercise the transposed cell-row chain + the
    # slice2d_norm normalize on non-leading (i≥2) cells.
    ...
    for (ci, (Ni, Nj, pat)) in enumerate(cases), χ in (16, 18)
        ...
        λc, ACc = ACenv_slice2d(AC_b, FL_b, M, FR_b, g; alg)
...
@testset "Gate C2: ACmap hoisting equivalence (sliced == dist)" begin
    ...
    M   = rand(ComplexF64, D, D, D, D, 2)
    ACb, FLb, FRb = slice2d_scatter(AC, g), slice2d_scatter(FL, g), slice2d_scatter(FR, g)
    χg = MPI.Allreduce(size(ACb, 1), +, g.col_comm); p_rs = split_ranges(χg, g.N1)
    FL_g = slice2d_gather_row(FLb, g, p_rs); FR_g = slice2d_gather_col(FRb, g, p_rs)
    @test ACmap_slice2d_sliced(ACb, FL_g, FR_g, M, g) ≈ ACmap_slice2d_dist(ACb, FLb, FRb, M, g) rtol=1e-13
    v_d, v_s = ACb, ACb
    for _ in 1:5
        v_d = ACmap_slice2d_dist(v_d, FLb, FRb, M, g)
        v_s = ACmap_slice2d_sliced(v_s, FL_g, FR_g, M, g)
    end
    @test v_s ≈ v_d rtol=1e-12
    Random.seed!(6701); Wb = slice2d_scatter(rand(ComplexF64, χ, D, D, χ), g)
    ld(ac, fl, fr) = real(sum(conj(Wb) .* ACmap_slice2d_dist(ac, fl, fr, M, g)))
    lh(ac, fl, fr) = real(sum(conj(Wb) .* ACmap_slice2d_sliced(ac, slice2d_gather_row(fl, g, p_rs), slice2d_gather_col(fr, g, p_rs), M, g)))
    gd = Zygote.gradient(ld, ACb, FLb, FRb); gh = Zygote.gradient(lh, ACb, FLb, FRb)
    for k in 1:3; @test gh[k] ≈ gd[k] rtol=1e-10; end
end

@testset "Gate C3: ACenv_slice2d gradient parity" begin
    g = slice2d_grid(2, 2)
    χ, D = 12, 2
    a_rs = split_ranges(χ, g.N1); i_rs = split_ranges(χ, g.N2)
    blkof(t) = t[a_rs[g.r1+1], :, :, i_rs[g.r2+1]]
    # 1×1 (no normalize), 2×1 (i=2 cell uses slice2d_norm — differentiate through it),
    ...
    for (ci, (Ni, Nj, mode)) in enumerate(((1, 1, :plain), (2, 1, :plain), (1, 1, :recompute)))
        ...
        loss_c(ac, fl, fr)   = real(sum(sum(conj(Wbs[c]) .* ACenv_slice2d(ac, fl, M, fr, g; alg)[2].data[c]) for c in 1:nu))
...
# ── Batch D: Cenv_slice2d (replicated-output; C full χ×χ, FL/FR gathered) ─────
# C is REPLICATED (not block-distributed) → compare it DIRECTLY (no slice2d_gather,
...
@testset "Gate D1: Cenv_slice2d eigenpair parity" begin
    g = slice2d_grid(2, 2)
    cases = [(1, 1, nothing), (1, 2, nothing), (2, 2, nothing), (2, 2, [1 2; 2 1])]
    for (ci, (Ni, Nj, pat)) in enumerate(cases), χ in (16, 18)
        D = 3
        ...
        λc, Cc = Cenv_slice2d(C, FL_b, FR_b, g; alg)    # C replicated (NOT scattered)
...
@testset "Gate D3: Cenv_slice2d gradient parity (take-my-block)" begin
    g = slice2d_grid(2, 2)
    χ, D = 12, 2
    a_rs = split_ranges(χ, g.N1); i_rs = split_ranges(χ, g.N2)
    blkof(t) = t[a_rs[g.r1+1], :, :, i_rs[g.r2+1]]
    ...
    loss_c(c, fl, fr)   = real(sum(conj(W) .* Cenv_slice2d(c, fl, fr, g; alg)[2].data[1]))
...
@testset "Gate E: hoisting reduces + flattens per-iteration allocation" begin
    ...
    ALu_row = slice2d_gather_row(ALub, g, l_rs); ALd_col = slice2d_gather_col(ALdb, g, a_rs)
    fh(v) = FLmap_slice2d_sliced(v, ALu_row, ALd_col, M, g)   # hoisted: gathers OUTSIDE the loop
    fd(v) = FLmap_slice2d_dist(v, ALub, ALdb, M, g)           # per-call: re-gathers ALu/ALd each step
