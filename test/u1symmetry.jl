...
@testset "U1 Tensor with $atype{$dtype}" for atype in [Array], dtype in [Float64]
    ...
    # u1bulkdims division
    @test u1bulkdims(5,8; parts = 2) == [[3, 2], [4, 4]]
    @test u1bulkdims(5,8; parts = 3) == [[2, 2, 1], [3, 3, 2]]
    @test u1bulkdims(5,8; parts = 4) == [[2, 1, 1, 1], [2, 2, 2, 2]]
    for a = 5:8, b = 5:8
    ...
    # initial 
    @test randU1(atype, dtype, 6, 7, 5).size == (6,7,5)
    @test zerosU1(atype, dtype, 6, 7, 5).size == (6,7,5)
    @test IU1(atype, dtype, 6).size == (6,6)

	A = randU1(atype, dtype, 3,3,4)
