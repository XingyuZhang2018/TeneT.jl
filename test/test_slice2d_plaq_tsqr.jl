# Plaquette Slice2D forward-only TSQR seam (4 ranks, 2x2 grid, CPU).
# Run via: julia --project=. test/run_test_slice2d_plaq_tsqr.jl
...
using TeneT: slice2d_grid, slice2d_scatter, slice2d_gather, split_ranges,
             ...
             init_VUMPSRuntime_slice2d, ACCtoAL, ACCtoAL_tsqr_slice2d

MPI.Init()
const comm = MPI.COMM_WORLD
const rank = MPI.Comm_rank(comm)
@assert MPI.Comm_size(comm) == 4 "test_slice2d_plaq_tsqr.jl expects exactly 4 ranks"
...
@testset "Plaquette ACCtoAL TSQR seam parity" begin
    ...
    for (ci, pat) in enumerate(PATS)
        ...
        ALt, err_t = ACCtoAL_tsqr_slice2d(scatter_sa(AC, g), C, g)
...
@testset "Plaquette ACCtoAL TSQR seam gradient parity" begin
    ...
    for (ci, pat) in enumerate(PATS)
        ...
        ltsqr(ac, c) = let (al, _) = ACCtoAL_tsqr_slice2d(ac, c, g)
